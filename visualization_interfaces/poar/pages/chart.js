...
function drawresource(){//Functions to draw Processing chart for resource
var resource_values=[0,0,0,0],resource_labels=['Alive','Suspected','Dead','Absent'];

Ext.Ajax.request({
	type:'GET',
	url: API_URI+'resources.json',/*Get the information about the resources*/
	headers:{'Content-Type':'application/json'},
	params:{},
	success:function (result,request) {
...
/* Calculate the percentage of various kind of jobs */
	...
	var add_send="";
	add_send+=resource_values[0];
	var process_i=0;
	for(process_i=1;process_i<resource_values.length;process_i++)
	{
		add_send+=","+resource_values[process_i];
	}
	var send="";
	send+="size("+rchart_width+","+rchart_height+");background(175,224,230);smooth();noStroke(); int diameter = 300;int[] angs = {"+add_send+"};float lastAng = 0;for (int i=0; i<angs.length; i++){fill(angs[i] * 3.0);arc(width/2, height/2, diameter, diameter, lastAng, lastAng+radians(angs[i]));lastAng += radians(angs[i]);  }arc(width/2, height/2, diameter, diameter, lastAng, lastAng+radians(angs[0]));";
	new Processing(rchart_el, send);

	setTimeout('drawresource()',5000);//Time after which it should refresh the chart
				...
				,listeners:{
					render:{
						...
						,fn:function(){
							...
							rchart_height=resourcePanel.getHeight();
							rchart_width=resourcePanel.getWidth();
							drawresource();
						}
					}
				}
