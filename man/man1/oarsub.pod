...
Specify the files that will store the standart output stream of the job. The %jobid% pattern is automatically replaced.

=item B<-E> <FILE>

Specify the files that will store the standart error stream of the job. The %jobid% pattern is automatically replaced.
