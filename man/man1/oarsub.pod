...
Use a job key. Parameter is the pathname to the secret key, public key is guessed (.pub is appended). If file does not already exist, generate it first (you can use %jobid% in the name and it will be replaced by the job id).
