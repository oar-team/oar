...
Activate the job-key mechanism. A job-key will be generated allowing to connect the job from outside the set of resources managed by OAR.

=item B<-i, --import-job-key-from-file>

Import the job-key to use from existing files (public and private key files) instead of generating a new one.

=item B<--import-job-key-inline-pub> and B<--import-job-key-inline-priv>

Import the job-key to use passing both the public and the private parts in the command line instead of generating a new one.

=item B<--job-key-file>

Specify the prefix of the files that will store the job-key for the job. The %jobid% pattern is automatically replaced.

=item B<--stdout-file> <FILE> B<--stderr-file>

Specify the files that will store the standart output stream and standart error of the job. The %jobid% pattern is automatically replaced.
