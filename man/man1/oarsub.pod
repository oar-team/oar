...
(you can use %jobid% and it will be replaced by the job id)
...
Use a job key. If nothing else is specified then it generates a ssh key.

=item B<-i, --import-job-key-from-file>

Use this file for the private ssh key and the .pub for the public one.

=item B<--gen-job-key-file>

Tell oarsub to generate the ssh key automatically(the %jobid% pattern is replaced by the job identifier)


=item B<--import-job-key-inline-priv> and B<--import-job-key-inline-pub>

Use a job key. Raw private and public key provided inline.
