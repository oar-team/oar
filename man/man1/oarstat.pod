...
B<oarstat> [-X|-Y|-J|-D|-f] [-j jobid|--array arrayid] [--sql SQL_properties] [-u user] [--array]
...
This command is used to show informations of jobs in OAR batch scheduler.
...
Print informations from a specific job.
...
Print only the state of a job. It makes a minimal query to the database. Must be used with -j.

=item B<-u, --user> [login]

Print informations for the current user or the one given.
If used with --accounting, print a more detailed accounting report for the user.

=item B<--array> [array_id]
 
Toggle the visualization of array information on.  If an array_id is provided, print informations relative to the subjobs of the given array job.
...
Shows accounting informations between two dates like "2006-03-30, 2006-04-30".
