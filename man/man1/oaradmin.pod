...
With -a option <EXP> form is as follows : /property=value. Several <EXP> can be used : /property=value/property=value... Property field correspond to the name of a property in OAR database : cpu, core, switch for examples. A specific keyword "node" (or "nodes") is use to indicate nodes names. Order of <EXP> expressions is important : it describe the resources properties hierarchy in the cluster. Properties used in <EXP> must exist and must be created with oarproperty command before. '/' caracter is mandatory at the beginning of each expression, no space between two expressions. 
...
Some expressions does not match or are not allowed :
  ...
  -a /node=cluster_a{12},cluster_b{12}. Only one argument must be used with {} form in each <EXP>.
...
Form for <EXP> is property=value. As add feature, property field correspond to the name of a property in OAR database already created with oarproperty command. With -s option, only one <EXP> expression can be specified. The boolean operators AND and OR cannot be used. With -p option, several <EXP> expressions can be used. When digits are used in host names, the expression -s node=mycluster[%3d1-8] must be used instead of -s node=mycluster[001-008].
...
Oaradmin ressources subcommand uses oarnodes outputs with yaml format. So, Perl yaml and Ruby yaml packages must be installed.
