require '/home/kameleon/lib/oarrestapi_lib.rb' 
...
@obj = OarApi.new(apiuri) #Object of OarApi class
end

def headfoot(body)
header = "describe \"OarApi\" do" 
footer = "end" 
str = header+"\n"+body+"\n"+footer
...
def test_job_notin_queue(jid)
 ...
 begin
    jobid = jid
    system("sleep 100")   # This is arbitrary as queue will be freed only based on its queue contents.. So might have to increase sleeping when queue is busy
    @obj.full_job_details
...
#objct = Tests.new("http://kameleon:kameleon@localhost/oarapi-priv")
