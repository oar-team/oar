...
This function is performed by :ref:`oarsub-anchor` with the --ssh_private_key
...
same ssh keys are used with each :ref:`oarsub-anchor`.

So a grid reservation (``-r`` option of :ref:`oarsub-anchor` on each OAR batch
...
Jobs can be suspended with the command :ref:`oarhold-anchor` (send a
``SIGSTOP`` on every processes on every nodes) to allow other jobs to be
executed.

``Suspended`` jobs can be resumed with the command :ref:`oarresume-anchor`
...
(:ref:`oarsub-anchor` ``-t`` option) and its exit code is equal to 99 then
...
You can also use :ref:`oardel-anchor` command to send the signal.
...
General steps used to schedule a job:
  ...
  2. All resources that match property constraints of the job(``-p`` option and
     ...
     :ref:`oarsub-anchor`) are stored in a tree data structure according to
...
This section explains how the ``--notify`` :ref:`oarsub-anchor` option is
handled by OAR:
 ...
 - The user wants to launch a script: The syntax is ``exec:/path/to/script
   ...
   :ref:`oarsub-anchor` command was invoked and then launches the script with
...
:ref:`oarsub-anchor` to specify that this is a besteffort job.

Important : a besteffort job cannot be a reservation.

If your job is of the type *besteffort* and *idempotent*
(:ref:`oarsub-anchor` ``-t`` option) and killed by the OAR scheduler then
another job is automatically created and scheduled with same behaviours.
...
:ref:`oarsub-anchor` to specify that this is a cosystem job.

These jobs are stopped by the :ref:`oardel-anchor` command or when they reach
...
:ref:`oarsub-anchor` to specify that this is a deploy job.
...
can use the :ref:`oarsub-anchor` option ``stagein`` to migrate your data.
