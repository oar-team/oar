...
my $Security_pam_file = "/etc/security/access_cluster.conf";
my $Security_pam_file_tmp = "$ENV{HOME}/access_cluster.conf";
...
# Be careful with the physical_package_id. Is it corresponding to the memory banc?
    ...
    if (!defined($Cpuset->{types}->{timesharing})){
        ...
        if (! system('diff /dev/cpuset/'.$Cpuset_path.$Cpuset_name.'/cpus /dev/cpuset/cpus > /dev/null 2>&1')){
