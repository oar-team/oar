...
let schedule_id_jobs_ct jids h_jobs h_slots =
  ...
  let rec assign_res_jobs j_ids scheduled_jobs = match j_ids with
		...
		| jid::n -> let j = find_job jid in
                let (test_inner, value) = test_type j "inner" in
                let num_set_slots = if test_inner then (int_of_string value) else 0 in
                  begin
                    ...
                    let (job, updated_slots ) = assign_resources_job_split_slots j (find_slots num_set_slots) in Hashtbl.replace h_slots 0 updated_slots;
  ...
  let rec loop_jobs od_id_jobs = match od_id_jobs with
    ...
    | jid::m ->  let j = find_job jid in
                ...
                begin
                  ...
                  Hashtbl.replace h_slots (int_of_string value) (split_slots_prev_scheduled_jobs (find_slots num_set_slots) [j]);
                  loop_jobs m
                end  
  in
