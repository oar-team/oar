...
#Require kernel headers (linux-headers)
...
MASTER_OAR="genepi-6.grenoble.grid5000.fr"
MASTER_DB="genepi-21.grenoble.grid5000.fr"
BACKUP_OAR="genepi-29.grenoble.grid5000.fr"
BACKUP_DB="genepi-31.grenoble.grid5000.fr"
...
####-------------------CGI script--------------------#####
#enable cgi script for monitor heartbeat ? require Apache
#"y" or "n"
CGI="y"
...
if [ "$ISDATABASE" = "y" ]; then
	...
	#installation test
	exists module-assistant
	module-assistant auto-install drbd8
fi

apt-get -y install $HEARTBEAT2 host
...
if [ "$ISMASTER" = "y" ]; then
	if [ "$ISDATABASE" = "y" ]; then
		ipmaster_oar=$(host $MASTER_OAR | cut -f3)
		ipmaster_db=$(host `uname -n` | cut -f3)	#don't use ifconfig beacause can beug if it is a non english system
		ipbackup_oar=$(host $BACKUP_OAR | cut -f3)
		ipbackup_db=$(host $BACKUP_DB | cut -f3)		
	elif [ "$ISDATABASE" = "n" ]; then
		ipmaster_oar=$(host `uname -n` | cut -f3)
		ipmaster_db=$(host $MASTER_DB | cut -f3)
		ipbackup_oar=$(host $BACKUP_OAR | cut -f3)
		ipbackup_db=$(host $BACKUP_DB | cut -f3)
	fi	
	...
	if [ "$ISDATABASE" = "y" ]; then
		...
		ipmaster_db=$(host $MASTER_DB | cut -f3)
		ipbackup_oar=$(host $BACKUP_OAR | cut -f3)
		ipbackup_db=$(host `uname -n` | cut -f3)	
	elif [ "$ISDATABASE" = "n" ]; then
		ipmaster_oar=$(host $MASTER_OAR | cut -f3)
		ipmaster_db=$(host $MASTER_DB | cut -f3)
		ipbackup_oar=$(host `uname -n` | cut -f3)
		ipbackup_db=$(host $BACKUP_DB | cut -f3)
	fi
else
	exit 1
...
if [ "$CONF" = "2nodes" ]; then
	...
	#experimental
	echo "ucast eth0_rename 192.168.0.1" >> /etc/ha.d/ha.cf
	echo "ucast eth0_rename 192.168.0.2" >> /etc/ha.d/ha.cf
...
#-----------------------Rename Database service for HA ----------------------------
...
if [ "$CONF" = "4nodes" ]; then
	...
	echo '         </primitive>' >> /var/lib/heartbeat/crm/cib.xml
	if [ "$BD" = "mysql" ]; then
		echo '         <primitive id="MysqlHA" class="lsb" type="mysqlha" provider="heartbeat">' >> /var/lib/heartbeat/crm/cib.xml
		echo '		 <operations>' >> /var/lib/heartbeat/crm/cib.xml
		echo '		   <op id="MysqlHA_mon" interval="120s" name="monitor" timeout="60s"/>' >> /var/lib/heartbeat/crm/cib.xml
		echo '		 </operations>' >> /var/lib/heartbeat/crm/cib.xml
		echo '           <meta_attributes id="MysqlHA_meta_attrs">' >> /var/lib/heartbeat/crm/cib.xml
		echo '             <attributes>' >> /var/lib/heartbeat/crm/cib.xml
		echo '               <nvpair id="MysqlHA_metaattr_target_role" name="target_role" value="started"/>' >> /var/lib/heartbeat/crm/cib.xml
		echo '             </attributes>' >> /var/lib/heartbeat/crm/cib.xml
		echo '           </meta_attributes>' >> /var/lib/heartbeat/crm/cib.xml
		echo '         </primitive>' >> /var/lib/heartbeat/crm/cib.xml
	elif [ "$BD" = "postgresql" ]; then
		echo "		<primitive id=\"Postgres\" class=\"lsb\" type=\"$BD-"$PGVERSION"ha\" provider=\"heartbeat\"" >> /var/lib/heartbeat/crm/cib.xml
		echo '		 <operations>' >> /var/lib/heartbeat/crm/cib.xml
		echo '		   <op id="Postgres_mon" interval="120s" name="monitor" timeout="60s"/>' >> /var/lib/heartbeat/crm/cib.xml
		echo '		 </operations>' >> /var/lib/heartbeat/crm/cib.xml
		echo '           <meta_attributes id="Postgres_meta_attrs">' >> /var/lib/heartbeat/crm/cib.xml
		echo '             <attributes>' >> /var/lib/heartbeat/crm/cib.xml
		echo '               <nvpair id="Postgres_metaattr_target_role" name="target_role" value="started"/>' >> /var/lib/heartbeat/crm/cib.xml
		echo '             </attributes>' >> /var/lib/heartbeat/crm/cib.xml
		echo '           </meta_attributes>' >> /var/lib/heartbeat/crm/cib.xml
		echo '         </primitive>' >> /var/lib/heartbeat/crm/cib.xml
	else
	...
	echo '         </primitive>' >> /var/lib/heartbeat/crm/cib.xml
	if [ "$BD" = "mysql" ]; then
		echo '         <primitive class="lsb" id="mysqlha" provider="heartbeat" type="mysqlha">' >> /var/lib/heartbeat/crm/cib.xml
		echo '           <operations>' >> /var/lib/heartbeat/crm/cib.xml
		echo '             <op id="mysqlha_mon" interval="120s" name="monitor" timeout="60s"/>' >> /var/lib/heartbeat/crm/cib.xml
		echo '           </operations>' >> /var/lib/heartbeat/crm/cib.xml
		echo '         </primitive>' >> /var/lib/heartbeat/crm/cib.xml
...
if [ "$ISDATABASE" = "y" ]; then
	if [ "$BD" = "mysql" ]; then
		...
		sed -e "s/bind-address/# bind-address/g" /etc/mysql/my.cnf > /etc/mysql/my.cnf.tmp && mv -f /etc/mysql/my.cnf.tmp /etc/mysql/my.cnf
	elif [ "$BD" = "postgresql" ]; then
...
if [ "$CGI" = "y" ]; then
	...
	echo '#!/bin/bash' > /usr/lib/cgi-bin/ha-status.cgi
	echo 'sudo /usr/sbin/crm_mon -w' >> /usr/lib/cgi-bin/ha-status.cgi
