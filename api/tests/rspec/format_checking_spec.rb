...
describe OarApi do
  ...
  describe "JOBS CHECKINGS: /jobs data structure" do
    ...
    context "(using state=Running,Launching,Waiting&limit=2)" do
      ...
      it "should return a correct self link" do
        @api.get_link_href('self').should == "/jobs?state=Running%2CLaunching%2CWaiting&limit=2&offset=0"
      end
      it "should return a correct next link" do
        @api.get_link_href('next').should == "/jobs?state=Running%2CLaunching%2CWaiting&limit=2&offset=2"
      end
    end
    ...
    context "(using /jobs?state=Running&limit=2&offset=3)" do
      ...
      it "should return a correct self link" do
        @api.get_link_href('self').should == "/jobs?state=Running%2CWaiting%2CLaunching&limit=2&offset=3"
      end
      it "should return a correct previous link" do
        @api.get_link_href('previous').should == "/jobs?state=Running%2CWaiting%2CLaunching&limit=2&offset=1"
      end
      it "should return a correct next link" do
        @api.get_link_href('next').should == "/jobs?state=Running%2CWaiting%2CLaunching&limit=2&offset=5"
      end
    end
    ...
    context "(with state=Running [note: add a sleep if test jobs are not yet running])" do
      ...
      it "should return jobs having a correct self link" do
        ...
        @api.get_link_href_from_array(links,"self").should == "/jobs/#{id}"
      end
      it "should return jobs having a correct resources link" do
        links=@api.value['items'][0]['links']
        id=@api.value['items'][0]['id']
        @api.get_link_href_from_array(links,"resources").should == "/jobs/#{id}/resources"
      end
  ...
  describe "RESOURCES CHECKING: /resources" do
    ...
    context "(with limit=2)" do
      ...
      it "should return a correct self link" do
        @api.get_link_href('self').should == "/resources?limit=2&offset=0"
      end
      it "should return a correct next link" do
        @api.get_link_href('next').should == "/resources?limit=2&offset=2"
      end
    end
    ...
    context "(with limit=2&offset=2)" do
      ...
      it "should return a correct self link" do
        @api.get_link_href('self').should == "/resources?limit=2&offset=2"
      end
      it "should return a correct next link" do
        @api.get_link_href('next').should == "/resources?limit=2&offset=4"
      end
      it "should return a correct previous link" do
        @api.get_link_href('previous').should == "/resources?limit=2&offset=0"
      end
    end
  ...
  describe "Job submission" do
    it "should return a self link" do
      ...
      @api.get_link_href('self').should == "/jobs/#{$ljobid}"
    end
  ...
  describe "Re-submission of a terminated job" do
    ...
    it "should have a parent link" do
      @api.get_link_href('parent').should == "/jobs/#{$jobid+1}"
    end
