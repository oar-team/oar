...
let get_job_list db resource_list =
(*
  let get_resources_for_job contraints =
     if constraints = "" then 
        resource_list
     else
        failwith "TODO"
...
*)
  let query = "
    SELECT jobs.job_id, moldable_job_descriptions.moldable_walltime, moldable_job_descriptions.moldable_id, job_resource_descriptions.res_job_value
...
(* OAR::IO::get_gantt_scheduled_jobs *)
