...
if ($ARGV[0] eq "init") {
    ...
    if (defined($Cpuset->{job_uid})) {
        ...
        if (
            system(
                "oardodo /usr/sbin/adduser --disabled-password --gecos 'OAR temporary user' --no-create-home --force-badname --quiet --home $tmp[7] --gid $tmp[3] --shell $tmp[8] --uid $Cpuset->{job_uid} $Cpuset->{job_user}"
            )
        ) {
            exit_myself(15,
                "Failed to create $Cpuset->{job_user} with uid $Cpuset->{job_uid} and home $tmp[7] and group $tmp[3] and shell $tmp[8]"
            );
        }
    }
        ...
        if (
            system(
                'oardodo mkdir -p /dev/cpuset/' .
                  ...
                  $Cpuset_path_job . ' && ' . 'oardodo "echo 0 > /dev/cpuset/mem_exclusive" && ' .
            ...
            #                    'cat /dev/cpuset/mems > /dev/cpuset/'.$Cpuset_path_job.'/mems && '.
                  'for c in ' .
                  join(" ", @Cpuset_cpus) .
                  ';do pnode=$(basename $(dirname $(find /sys/devices/system/node -name cpu$c))); pmems="$pmems,${pnode#node}";done && '
                  . 'echo ${pmems#,} > /dev/cpuset/'
