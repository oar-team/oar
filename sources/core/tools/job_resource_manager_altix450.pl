...
# Be careful with the physical_package_id. Is it corresponding to the memory banc?
        if (system( 'oardodo mkdir -p /dev/cpuset/'.$Cpuset_path_job.' && '.
                    ...
                    'oardodo "echo 0 > /dev/cpuset/mem_exclusive" && '.
...
#                    'cat /dev/cpuset/mems > /dev/cpuset/'.$Cpuset_path_job.'/mems && '.
                    'for c in '.join(" ",@Cpuset_cpus).';do pnode=$(basename $(dirname $(find /sys/devices/system/node -name cpu$c))); pmems="$pmems,${pnode#node}";done && '.
                    'echo ${pmems#,} > /dev/cpuset/'.$Cpuset_path_job.'/mems && '.
