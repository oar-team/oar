...
my $Db = DBI->connect("DBI:$Db_type:database=$Db_name;host=$Db_host",
    $Db_user, $Db_pass, { 'InactiveDestroy' => 1, 'PrintError' => 1 });

my $sth = $Db->prepare(
    "    SELECT window_stop, network_address, subvalue
...
system(
    'mencoder "mf://*.png" -mf fps=20:w=1000:h=1000:type=png -o cpuset_cpu.avi -ovc lavc -lavcopts vcodec=mpeg4'
);
