#!/usr/bin/perl
# Prologue script for OAR/PBS besteffort coupling
...
my @NODES        = (
    "healthphy[1]",    "healthphy[2]",    "healthphy[3]",    "healthphy[4]",
    "healthphy[5]",    "healthphy[6]",    "healthphy[7]",    "healthphy[8]",
    "healthphy[9]",    "healthphy[10]",   "healthphy[11]",   "healthphy[12]",
    "healthphy[13]",   "healthphy[14]",   "healthphy[15]",   "healthphy[16]",
    "healthphy[17]",   "healthphy-xeon1", "healthphy-xeon2", "healthphy-xeon3",
    "healthphy-xeon4", "healthphy-xeon5", "healthphy-xeon6", "healthphy-xeon7");
        ...
        if ($key eq "jobs") {
            ...
            foreach my $job (@jobs) {
                ...
                if ($j eq $ARGV[0]) {
                    ...
                    `$OARNODESETTING_CMD -r $second_resource -s Absent`;
                    #########
                }
            }
        }
