...
sub print_gantt($){
    my $gantt = shift;
    print("--------------------------------------------------------------------\n");
    print(unpack("b*", $gantt->[0]->[2])."\n");
    foreach my $g (@{$gantt}){
        print("BEGIN : $g->[0](".strftime("%F %T",localtime($g->[0])).")\n");
        foreach my $h (@{$g->[1]}){
            print("    $h->[0](".strftime("%F %T",localtime($h->[0]))."): ".unpack("b*", $h->[1])."\n");
        }
    }
    print("--------------------------------------------------------------------\n");
}
...
sub print_strips($){
    ...
    foreach my $s (sort {$a <=> $b} keys(%$strips)) {
        print("$s(".strftime("%F_%T",localtime($s))."): ".unpack("b*", $strips->{$s})."\n");
    }
    print("--------------------------------------------------------------------\n");
}
...
sub compute_constraints($$$) {
    ...
    foreach (split(/\s*,\s*/,$constraint_str)) {
        if (/^(\d+)\/(\d?\d):(\d\d)\/(\d+)(?::(\d\d))?$/) {
            ...
            my @days = keys { map { $_ => 1 } split(//,$c->{days}) }; # make days unique
            foreach my $d (@days) {
                my $start = $now->clone();
...
sub manage_gantt_hierarchy($$$$$$) {
    ...
    if (defined($types->{inner})){
        ...
        # $constraint should only be defined if we are in a scheduling phase. Computing gantt with constraints is indeed useless when handling jobs which are already scheduled.
        $gantt_name = "constraints:$types->{constraints}";
        oar_debug("$log_prefix constraint job, using gantt: ($gantt_name,...)\n");
    }
    if (defined($types->{placeholder})){ # the placeholder job type is incompatible with granted and timesharing types 
    ...
    #print_gantt( $gantt->{$gantt_name}->{$allowed_name}->{$timesharing_name}->{$timesharing_user});
