            ...
            foreach my $n (keys(%{$Gantt->{$inner_id}->{$p}->{$u}})){
                if (not (($p ne "" and $p eq $placeholder_name) or ($u ne "" and $u eq $timesharing_user and $n ne "" and $n eq $timesharing_name))){
                    ...
                    if ($placeholder_name ne "") {
                        oar_debug($module_name, "skip job occupation in gantt ($inner_id,$p,$u,$n) because job is ($inner_id,$placeholder_name,,)\n", $session_id, $job_id);
                    } else {
                        oar_debug($module_name, "skip job occupation in gantt ($inner_id,$p,$u,$n) because job is ($inner_id,$allowed_name,$timesharing_user,$timesharing_name)\n", $session_id, $job_id);
                    }
                }
            }
        }
