            ...
            foreach my $n (keys(%{ $Gantt->{$inner_id}->{$p}->{$u} })) {
                if (
                    ...
                    oar_debug($module_name, "add job occupation in gantt ($inner_id,$p,$u,$n)\n",
                        $session_id, $job_id);
                    set_occupation($Gantt->{$inner_id}->{$p}->{$u}->{$n},
                        $date, $duration, $resources_vec);
                } else {
                    if ($placeholder_name ne "") {
                        oar_debug(
                            $module_name,
                            "skip job occupation in gantt ($inner_id,$p,$u,$n) because job is ($inner_id,$placeholder_name,,)\n",
                            $session_id,
                            $job_id);
                    } else {
                        oar_debug(
                            $module_name,
                            "skip job occupation in gantt ($inner_id,$p,$u,$n) because job is ($inner_id,$allowed_name,$timesharing_user,$timesharing_name)\n",
                            $session_id,
                            $job_id);
                    }
                }
...
#print("Treate hole $current_hole_index, $h: $gantt->[$current_hole_index]->[0] --> $gantt->[$current_hole_index]->[1]->[$h]->[0]\n");
            ...
            do {
                foreach my $l (OAR::Schedulers::ResourceTree::get_tree_leafs($tree_clone)) {
                    vec($already_occupied_resources_vec,
                ...
                foreach my $l (OAR::Schedulers::ResourceTree::get_tree_leafs($tree_clone)) {
                    if (
                        (
                            !vec(
                                $gantt->[$current_hole_index]->[1]->[$h]->[1],
                                OAR::Schedulers::ResourceTree::get_current_resource_value($l), 1)
                        ) or
                        (
                            vec($already_occupied_resources_vec,
                ...
                $tree_clone =
                  OAR::Schedulers::ResourceTree::delete_tree_nodes_with_not_enough_resources(
                    $tree_clone);
                $tree_clone =
                  OAR::Schedulers::ResourceTree::delete_unnecessary_subtrees($tree_clone);
...
#print("Treate hole $current_hole_index, $h: $gantt->[$current_hole_index]->[0] --> $gantt->[$current_hole_index]->[1]->[$h]->[0]\n");
            ...
            while ((defined($result_children[$process_index_to_check])) and ($end_loop == 0)) {
                if (defined($result_children[$process_index_to_check]->{result_tree_list})) {
                    ...
                    @result_tree_list =
                      @{ $result_children[$process_index_to_check]->{result_tree_list} };
                    $end_loop = 1;
                }
                $current_process_hole_index =
                  $result_children[$process_index_to_check]->{current_hole_index};
            ...
            if (($end_loop == 0) and
                ($current_process_hole_index <= $#{$gantt}) and
                (   ((time() - $timeout_initial_time) >= $timeout) or
                    (   ($gantt->[$current_process_hole_index]->[0] == $gantt->[0]->[5]->[0]) and
                        ($gantt->[$current_process_hole_index]->[1]->[$h]->[0] >=
                            $gantt->[0]->[5]->[1])
                    ) or
                    ($gantt->[$current_process_hole_index]->[0] > $gantt->[0]->[5]->[0])
                ) and
