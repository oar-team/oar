...
sub new_with_1_hole($$$$$$){
    ...
    my $gantt = OAR::Schedulers::GanttHoleStorage::new($max_resource_number, $minimum_hole_duration);
...
sub manage_gantt_for_timesharing_and_placeholder($$$$$$) {
    ...
    my $timesharing_user = "";
    my $timesharing_name = "";
    if (defined($types->{placeholder})){ # A set_placeholder job cannot be use_placeholder or timesharing. 
        $placeholder_name = $types->{placeholder};
        oar_debug("$log_prefix job is ($inner_id,s:$placeholder_name,,)\n");
        if (not defined($Gantt->{$inner_id}->{$placeholder_name}->{""}->{""})){
            $Gantt->{$inner_id}->{$placeholder_name}->{""}->{""} = dclone($Gantt->{$inner_id}->{""}->{""}->{""});
            oar_debug("$log_prefix set_placeholder job: cloned gantt ($inner_id,$placeholder_name,,) from ($inner_id,,,)\n");
        }
    } else {
        ...
        oar_debug("$log_prefix job is ($inner_id,u:$allowed_name,$timesharing_user,$timesharing_name)\n");
        if (not defined($Gantt->{$inner_id}->{$allowed_name}->{$timesharing_user}->{$timesharing_name})) {
            if (not defined($Gantt->{$inner_id}->{$allowed_name}->{""}->{""}) and not defined($Gantt->{$inner_id}->{""}->{$timesharing_user}->{$timesharing_name})) {
                $Gantt->{$inner_id}->{$allowed_name}->{$timesharing_user}->{$timesharing_name} = dclone($Gantt->{$inner_id}->{""}->{""}->{""});
                oar_debug("$log_prefix use_placeholder/timesharing job: cloned gantt ($inner_id,$allowed_name,$timesharing_user,$timesharing_name) from ($inner_id,,,)\n");
                if ($allowed_name ne "") {
                    $Gantt->{$inner_id}->{$allowed_name}->{""}->{""} = dclone($Gantt->{$inner_id}->{""}->{""}->{""});
                    oar_debug("$log_prefix use_placeholder/timesharing job: cloned gantt ($inner_id,$allowed_name,,) from ($inner_id,,,)\n");
                }
            ...
            } elsif (not defined($Gantt->{$inner_id}->{$allowed_name}->{""}->{""})) { #G($i,,$u,$n) is defined
                $Gantt->{$inner_id}->{$allowed_name}->{""}->{""} = dclone($Gantt->{$inner_id}->{""}->{""}->{""});
                oar_debug("$log_prefix use_placeholder/timesharing job: cloned gantt ($inner_id,$allowed_name,,) from ($inner_id,,,)\n");
                if ($timesharing_user ne "" and $timesharing_name ne "") {
                    ...
                    oar_debug("$log_prefix use_placeholder/timesharing job: cloned gantt ($inner_id,$allowed_name,$timesharing_user,$timesharing_name) from ($inner_id,,$timesharing_user,$timesharing_name)\n");
                }
                ...
                if ($allowed_name ne "") {
                    ...
                    oar_debug("$log_prefix use_placeholder/timesharing job: cloned gantt ($inner_id,$allowed_name,$timesharing_user,$timesharing_name) from ($inner_id,$allowed_name,,)\n");
                }
            } else { # Both G($i,$p,,) and G($i,,$u,$n) are defined. We need to merge them to create G($i,$p,$u,$n) 
                $Gantt->{$inner_id}->{$allowed_name}->{$timesharing_user}->{$timesharing_name} = OAR::Schedulers::GanttHoleStorage::merge_clone($Gantt->{$inner_id}->{$allowed_name}->{""}->{""},$Gantt->{$inner_id}->{""}->{$timesharing_user}->{$timesharing_name});
                oar_debug("$log_prefix use_placeholder/timesharing job: merged gantt ($inner_id,$allowed_name,$timesharing_user,$timesharing_name) from ($inner_id,$allowed_name,,) and ($inner_id,,$timesharing_user,$timesharing_name)\n");
            }
        }
    }
