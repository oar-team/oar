  ...
  def self.invoke command, *args
    ...
    when 0
      ...
      opts = OptionParser.new do |opts|
        ...
        # auto_offset
        opts.on("--auto-offset", "Automatically compute the node, cpu, and core offsets for you based on what is already registered in the OAR database") do
      ...
      if $options[:add]
        ...
        # FIXME: I guess there is a bug here..
        if [:cpusetproperty]
          i=0
          while i<ARGV.length
...
if __FILE__ == $0
  ...
  $subcommand.each_index do |i|
    command = i if !$subcommand[i].nil? && !ARGV[0].nil? && (ARGV[0]==$subcommand[i][:short_form] || ARGV[0]==$subcommand[i][:long_form])
  end
  if command == -1
    $stderr.puts "Incoherence in specified subcommand" if !ARGV[0].nil?
    CLI.subcommand_usage
  end
