...
sub oar_log($$$$;$) {
    ...
    $string = sprintf($log_format,
        $level, strftime("%FT%T", localtime($seconds)) . ".$microseconds",
        $module_name, $session_id, defined($job_id) ? $job_id : "", $string);
    write_log($string);
}
...
#OAR::IO::disconnect($base);
    oar_info(
        $Module_name,
        "Mail ERROR: $job_id $error --> SMTP server used: $smtpServer, sender: $mailSenderAddress, recipients: $mailRecipientAddress, object: $object, body: $body\n",
        $Session_id);
    exit(1);
}
...
sub send_mail($$$$) {
    ...
    if (!defined($smtp_server) ||
        !defined($mail_sender_address) ||
        !defined($mail_recipient_address)) {
    ...
    if ($pid == 0) {
        ...
        my $smtp = Net::SMTP->new(
          ...
          treate_mail_error($smtp_server, $mail_sender_address, $mail_recipient_address, $object,
            $body, "No SMTP connexion", $job_id);
        $smtp->mail($mail_sender_address) or
          treate_mail_error($smtp_server, $mail_sender_address, $mail_recipient_address, $object,
            $body, "MAIL FROM", $job_id);
        my @recipients = split(',', $mail_recipient_address);
        $smtp->to(@recipients) or
          treate_mail_error($smtp_server, $mail_sender_address, $mail_recipient_address, $object,
            $body, "RCPT TO", $job_id);
        $smtp->data() or
          treate_mail_error($smtp_server, $mail_sender_address, $mail_recipient_address, $object,
        ...
        $smtp->datasend("To: $mail_recipient_address\n") or
          treate_mail_error($smtp_server, $mail_sender_address, $mail_recipient_address, $object,
            $body, "Cannot send", $job_id);
        $smtp->datasend("Subject: $object\n") or
          treate_mail_error($smtp_server, $mail_sender_address, $mail_recipient_address, $object,
            $body, "Cannot send", $job_id);
        $smtp->datasend($body) or
          treate_mail_error($smtp_server, $mail_sender_address, $mail_recipient_address, $object,
            $body, "Cannot send", $job_id);
        $smtp->dataend() or
          treate_mail_error($smtp_server, $mail_sender_address, $mail_recipient_address, $object,
            $body, "DATA END", $job_id);
        $smtp->quit or
          treate_mail_error($smtp_server, $mail_sender_address, $mail_recipient_address, $object,
            $body, "QUIT", $job_id);
        exit(0);
...
sub notify_user($$$$$$$$) {
    ...
    if ($method =~ m/^.*mail\s*:(.+)$/m) {
        OAR::IO::add_new_event($base, "USER_MAIL_NOTIFICATION", $job_id,
            "[Judas] Send a mail to $1 --> $tag");
        send_mail($1, "*OAR* [$tag]: $job_id ($job_name) on $Instance_name", $comments, $job_id);
    } elsif ($method =~ m/^.*exec\s*:([a-zA-Z0-9_.\/ -]+)$/m) {
        my $cmd =
          "$Openssh_cmd -x -T $host OARDO_BECOME_USER=$user oardodo $1 $job_id $job_name $tag \\\"$comments\\\" > /dev/null 2>&1";
        $SIG{PIPE} = 'IGNORE';
        my $pid = fork();
        if ($pid == 0) {
            undef($base);
            ...
            if ($@) {
                ...
                my $dbh = OAR::IO::connect();
                my $str =
                  "Launched user notification command: $cmd; exit value = $exit_value, signal num = $signal_num, dumped core = $dumped_core";
            ...
            oar_error($Module_name,
                "Error when forking process to execute notify user command: $cmd\n", $Session_id);
        }
    } else {
        oar_debug($Module_name,
            "No correct notification method found ($method) for the job $job_id\n", $Session_id);
    }
}
