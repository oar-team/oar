...
sub add_child($$$$) {
    ...
    if (!defined($tree_ref->[1]->{$resource_value})) {
        ...
        $tmp_ref = [
            $tree_ref, undef, $resource_name, $resource_value, $tree_ref->[4] + 1,
            0, 0, undef, undef, undef, 0, undef ];
...
#print("Go to FATHER: ".get_current_resource_value($current_node)."\n") if (defined(get_current_resource_value($current_node)));
                if (
                    (   get_needed_children_number($current_node) >
                        get_current_children_number($current_node)
                    ) or
                    (   (get_needed_children_number($current_node) == -1)    # ALL
                        and
                        (get_max_available_children($current_node) >
                            get_current_children_number($current_node))
                    ) or
...
sub delete_unnecessary_subtrees($) {
    ...
    my $current_node = $tree_ref;
    do {
        if ((get_needed_children_number($current_node) >= 0) and
            (get_needed_children_number($current_node) < get_current_children_number($current_node))
        ) {
...
#            print("Go to CHILD: ".get_current_resource_name($current_node)."=".get_current_resource_value($current_node)."\n");
            ...
            # Treate leaf
            if (defined($authorized_leafs_vec) and
                (is_node_a_leaf($current_node) == 1)                        and
                ($tree_ref != $current_node)                                and
                (get_current_resource_name($current_node) eq "resource_id") and
                (!vec($authorized_leafs_vec, get_current_resource_value($current_node), 1))) {
...
# This leaf is not available
                ...
                while ($current_node and (!get_previous_brother($current_node))) {
                    if (
                        ...
                        (   ($tree_ref != $current_node) and
                            (get_needed_children_number(get_father($current_node)) > 0) and
                            ($nb_children_validated{ get_father($current_node) } >=
                                get_needed_children_number(get_father($current_node)))
                        ) or
                        (get_needed_children_number($current_node) ==
                            0)    # For stupid requests with 0 item wanted
                    ) {
                        # we want to delete the root
                        return (undef) if ($tree_ref == $current_node);
...
#                       print("Go to FATHER: ".get_current_resource_name($current_node)."=".get_current_resource_value($current_node)." ($nb_children_validated{$current_node}/".get_needed_children_number($current_node).")\n") if (defined(get_current_resource_value($current_node)));
                ...
                if ($current_node and
                    ($tree_ref != $current_node)) {
                    my $tmp_father = get_father($current_node);
                    if ((get_needed_children_number($tmp_father) > 0) and
                        ($nb_children_validated{$tmp_father} >=
                            get_needed_children_number($tmp_father))
                    ) {
