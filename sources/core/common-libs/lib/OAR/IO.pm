...
my @Job_states = (
    'Waiting',          'Hold',      'toLaunch',   'toError',
    'toAckReservation', 'Launching', 'Running',    'Suspended',
    'Resuming',         'Error',     'Terminated', 'Finishing');
...
my @Starting_to_running_job_states =
  array_minus(@Job_states, ('Waiting', 'Error', 'Terminated', 'Finishing'));
my @Starting_to_finishing_job_states =
  array_minus(@Job_states, ('Waiting', 'Error', 'toError', 'Terminated'));
...
sub connect_db($$$$$$) {
    ...
    if ($host =~ m/;/) {
        ...
        oar_error(
            $Module_name,
            "Error reading $oldHost attribute from OAR configuration file, using $host instead.\n",
            $Session_id);
    }
    if ($dbport =~ m/;/) {
        my $oldDbport = $dbport;
        ...
        oar_error(
            $Module_name,
            "Error reading $oldDbport attribute from OAR configuration file, using $dbport instead.\n",
            $Session_id);
    }
    if ($name =~ m/;/) {
        my $oldname = $name;
        ...
        oar_error(
            $Module_name,
            "Error reading $oldname attribute from OAR configuration file, using $name instead.\n",
            $Session_id);
    }
    if ($user =~ m/;/) {
        my $olduser = $user;
        ...
        oar_error(
            $Module_name,
            "Error reading $olduser attribute from OAR configuration file, using $user instead.\n",
            $Session_id);
    }
    ...
    if ($Db_type eq "Pg" || $Db_type eq "psql") {
        ...
        oar_error($Module_name,
            "Cannot recognize DB_TYPE tag \"$Db_type\". So we are using \"mysql\" type.\n",
            $Session_id);
        $type     = "mysql";
        $Db_type  = "mysql";
        $dbd_opts = ";mysql_local_infile=1";    #Mysql option to allow LOAD LOCAL INFILE
    ...
    my $dbh = DBI->connect($connection_string, $user, $pwd,
        { 'InactiveDestroy' => 1, 'PrintError' => $printerror });

    if (!defined($dbh)) {
        oar_error(
            $Module_name,
            "Cannot connect to database (type=$Db_type, host=$host, user=$user, database=$name): $DBI::errstr\n",
            $Session_id);
    }
    return ($dbh);
}
...
sub timeout_db($$) {
    ...
    if (!defined($dbh)) {
        oar_warn($Module_name,
            "I will retry to connect to the database in $Timeout_db_connection s\n", $Session_id);
        send_log_by_email("OAR database connection failed",
            "[IOlib] I will retry to connect to the database in $Timeout_db_connection s\n");
        my $sleep_time = 0;
        while ($sleep_time <= 1) {
...
sub get_possible_wanted_resources($$$$$$$) {
    ...
    if (
        defined(
            $TREE_CACHE_HASH->{$resource_tree_cache_key}->{$sql_where_string}->{$sql_in_string}
              ->{$order_part}->{$possible_resources_vector}->{$impossible_resources_vector})
    ) {
        #oar_info($Module_name, "Use tree cache to get ressource structure.\n", $Session_id);
        return (
            OAR::Schedulers::ResourceTree::clone(
                $TREE_CACHE_HASH->{$resource_tree_cache_key}->{$sql_where_string}->{$sql_in_string}
                  ->{$order_part}->{$possible_resources_vector}->{$impossible_resources_vector}));
    }
...
EOS
    ...
    while (my @sql = $sth->fetchrow_array()) {
        ...
        foreach (my $i = 0; $i <= $#wanted_resources; $i++) {
            ...
            $father_ref =
              OAR::Schedulers::ResourceTree::add_child($father_ref,
                $wanted_resources[$i]->{resource},
                $sql[$i], $result);
            ...
            OAR::Schedulers::ResourceTree::set_needed_children_number($father_ref,
                $wanted_children_number);

            # Verify if we must keep this child if this is resource_id resource name
            if ($wanted_resources[$i]->{resource} eq "resource_id") {
                if ((defined($impossible_resources_vector)) and
                    (vec($impossible_resources_vector, $sql[$i], 1))) {
                    OAR::Schedulers::ResourceTree::delete_subtree($father_ref);
                    $i = $#wanted_resources + 1;
                } elsif ((defined($possible_resources_vector)) and
                    (!vec($possible_resources_vector, $sql[$i], 1))) {
                    OAR::Schedulers::ResourceTree::delete_subtree($father_ref);
                    $i = $#wanted_resources + 1;
                }
            }
    ...
    $TREE_CACHE_HASH->{$resource_tree_cache_key}->{$sql_where_string}->{$sql_in_string}
      ->{$order_part}->{$possible_resources_vector}->{$impossible_resources_vector} = $result;
...
sub estimate_job_nb_resources($$$) {
    ...
    foreach my $moldable_resource (@{$ref_resource_list}) {
        my $tmp_moldable_result = {
            ...
            comment     => "no comment" };
        $tmp_moldable_result->{walltime} = $moldable_resource->[1]
        ...
        foreach my $r (@{ $moldable_resource->[0] }) {
            ...
            my $tree = get_possible_wanted_resources($dbh_ro, undef, $resource_id_list_vector,
                \@dead_resources, $tmp_properties, $r->{resources}, undef);
            $tree =
            ...
            if (!defined($tree)) {
                ...
                if ($DBI::errstr ne "") {
                    ...
                    $tmp_moldable_result->{comment} =
                      "There are not enough resources for your request";
                    $tmp_moldable_result->{nbresources} = 0;
                }
                last;
            } else {
                my ($tmp_leafs_vec, $tmp_leafs_hashref) =
                  OAR::Schedulers::ResourceTree::get_tree_leafs_vec($tree);
...
sub job_key_management($$$$$) {
    my ($use_job_key, $import_job_key_inline, $import_job_key_file, $export_job_key_file,
        $verbose_level)
      = @_;
    ...
    if (defined($use_job_key) and
        !($import_job_key_inline ne "") and
        !($import_job_key_file ne "")   and
        defined($ENV{OAR_JOB_KEY_FILE})) {
        $import_job_key_file = $ENV{OAR_JOB_KEY_FILE};
    }
    if ((!defined($use_job_key)) and
        (($import_job_key_inline ne "") or
            ($import_job_key_file ne "") or
            ($export_job_key_file ne ""))
    ) {
        warn(
            "# Error: you must set the --use-job-key (or -k) option in order to use other job key related options.\n"
        );
        return (-15, '', '');
    }
    if (defined($use_job_key)) {
        if (($import_job_key_inline ne "") and ($import_job_key_file ne "")) {
            warn(
                "# Error: you cannot import a job key both inline and from a file at the same time.\n"
            );
            return (-15, '', '');
        }
        ...
        if (($import_job_key_inline ne "") or ($import_job_key_file ne "")) {
            ...
            system(
                {"bash"} "bash",
                "-c",
                "SSH_ASKPASS=/bin/true ssh-keygen -y -f $tmp_job_key_file < /dev/null 2> /dev/null > $tmp_job_key_file.pub"
            );
            if ($? != 0) {
                warn(
            ...
            system({"bash"} "bash",
                "-c", 'ssh-keygen -b 1024 -N "" -t rsa -f "' . $tmp_job_key_file . '" > /dev/null');
            if ($? != 0) {
    ...
    if (defined($use_job_key)) {
        ...
        if ($job_key_pub !~ /^(ssh-rsa|ssh-dss)\s.+\n*$/) {
            warn(
                "# Error: bad job key format. The public key must begin with either `ssh-rsa' or `ssh-dss' and is only 1 line.\n"
            );
            return (-14, '', '');
        }
        $job_key_pub =~ s/\n//g;
...
sub add_micheline_job($$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$) {
    ...
    my $sth =
      $dbh->prepare("SELECT rule FROM admission_rules WHERE enabled = 'YES' ORDER BY priority,id");
    $sth->execute();
    while (my $ref = $sth->fetchrow_hashref()) {
    ...
    {    #to test  add_micheline_simple_array_job
        ...
        @Job_id_list = add_micheline_simple_array_job_non_contiguous(
            ...
            $queue_name,             $jobproperties,
            $startTimeReservation,   $idFile,
            $checkpoint,             $checkpoint_signal,
            $notify,                 $job_name,
            $job_env,                $type_list,
            $launching_directory,    $anterior_ref,
            $stdout,                 $stderr,
            $job_hold,               $project,
            $initial_request_string, $array_id,
        ...
        foreach my $command (@array_job_commands) {
            my ($err, $ssh_priv_key, $ssh_pub_key) =
              job_key_management($use_job_key, $import_job_key_inline, $import_job_key_file,
                $export_job_key_file, $verbose_level);
            if ($err != 0) {
...
sub format_job_message_text($$$$$$$$$) {
    my ($job_name, $estimated_nb_resources, $estimated_walltime,
        $job_type, $reservation,            $queue,
        $project,  $type_list_array_ref,    $string
    ) = @_;
    ...
    my $types_to_text = '';
    $types_to_text = "T=" . join('|', @{$type_list_array_ref}) . ","
      if ($#{$type_list_array_ref} >= 0);
    my $job_message =
      "R=$estimated_nb_resources,W=" . duration_to_sql($estimated_walltime) . ",J=$job_mode,";
...
sub add_micheline_subjob($$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$) {
    ...
    my $job_message =
      format_job_message_text($job_name, $estimated_nb_resources, $estimated_walltime, $jobType,
        $reservationField, $queue_name, $project, $type_list, '');
...
sub add_micheline_simple_array_job ($$$$$$$$$$$$$$$$$$$$$$$$$$$$) {
    my ($dbh,                    $dbh_ro,
        $jobType,                $ref_resource_list,
        $array_job_commands_ref, $infoType,
        $queue_name,             $jobproperties,
        $startTimeReservation,   $idFile,
        $checkpoint,             $checkpoint_signal,
        $notify,                 $job_name,
...
EOS
    ...
    foreach my $command (@{$array_job_commands_ref}) {
        $command    = $dbh->quote($command);
        $query_jobs = $query_jobs .
          "(\'$jobType\',\'$infoType\',\'Hold\',\'$user\',$command,\'$date\',\'$queue_name\',$jobproperties,$launching_directory,\'$reservationField\',\'$startTimeJob\',$idFile,$checkpoint,$job_name_quoted,$notify,\'$checkpoint_signal\',$stdout,$stderr,$job_env,$project,$initial_request_string,$array_id,$array_index,\'$job_message\'),";
        $array_index++;
    }
    ...
    my $random_number;
    my $query_challenges =
      "INSERT INTO challenges (job_id,challenge,ssh_private_key,ssh_public_key) VALUES ";

    my $moldable_resource = @{$ref_resource_list}[0];
    $moldable_resource->[1] = $Default_job_walltime if (!defined($moldable_resource->[1]));
    my $walltime = $moldable_resource->[1];
    my $query_moldable_job_descriptions =
      "INSERT INTO moldable_job_descriptions (moldable_job_id,moldable_walltime) VALUES ";

    for (my $i = 0; $i < $nb_jobs; $i++) {
        push(@Job_id_list, $job_id);
        $random_number    = int(rand(1000000000000));
        $query_challenges = $query_challenges .
          "($job_id,\'$random_number\',\'\',\'\'),";    #TODO $ssh_priv_key,$ssh_pub_key
        $query_moldable_job_descriptions =
    ...
    my $query_job_resource_groups =
      "INSERT INTO job_resource_groups (res_group_moldable_id,res_group_property) VALUES ";
    for (my $i = 0; $i < $nb_jobs; $i++) {
        foreach my $r (@{ $moldable_resource->[0] }) {
            my $property = $r->{property};
            $property                  = $dbh->quote($property);
    ...
    if ($pg) {
        $res_group_id -= $nb_resource_grp * $nb_jobs - 1;
    }    #TODO: add *nb_moldable for moldable support
    my $query_job_resource_descriptions =
      "INSERT INTO job_resource_descriptions (res_job_group_id,res_job_resource_type,res_job_value,res_job_order) VALUES ";

    for (my $i = 0; $i < $nb_jobs; $i++) {
        foreach my $r (@{ $moldable_resource->[0] }) {
            my $order = 0;
            foreach my $l (@{ $r->{resources} }) {
                $query_job_resource_descriptions = $query_job_resource_descriptions .
                  "($res_group_id,\'$l->{resource}\',\'$l->{value}\',$order),";
                $order++;
            }
            $res_group_id++;
        }
    ...
    if ($#{$anterior_ref} > 0) {
        $job_id = $first_array_job_id;
        my $query_job_dependencies =
          "INSERT INTO job_dependencies (job_id,job_id_required) VALUES ";
        for (my $i = 0; $i < $nb_jobs; $i++) {
            foreach my $a (@{$anterior_ref}) {
    ...
    if (defined($job_hold)) {
        $query_array_id =
          $query_array_id . " array_id = " . $first_array_job_id . " WHERE job_id IN (";
    } else {
        $query_array_id = $query_array_id .
          "state = \'Waiting\', array_id = " . $first_array_job_id . " WHERE job_id IN (";
        $state_log = "\'Waiting\'";
    }
...
EOS
    ...
    for (my $i = 0; $i < $nb_jobs; $i++) {
        foreach my $r (@{ $moldable_resource->[0] }) {
            ...
            foreach my $l (@{ $r->{resources} }) {
                $query_job_resource_descriptions = $query_job_resource_descriptions .
                  "($res_group_ids[$k],\'$l->{resource}\',\'$l->{value}\',$order),";
                $order++;
            }
            $k++;
        }
...
EOS
        ...
        if (($state eq "Terminated") or
            ($state eq "Error")     or
            ($state eq "toLaunch")  or
            ($state eq "Running")   or
            ($state eq "Suspended") or
            ($state eq "Resuming")) {
            my $job = get_job($dbh, $job_id);
            my ($addr, $port) = split(/:/, $job->{info_type});
            if ($state eq "Suspended") {
                OAR::Modules::Judas::notify_user($dbh, $job->{notify}, $addr, $job->{job_user},
                    $job->{job_id}, $job->{job_name}, "SUSPENDED", "Job is suspended.");
            } elsif ($state eq "Resuming") {
                OAR::Modules::Judas::notify_user($dbh, $job->{notify}, $addr, $job->{job_user},
                    $job->{job_id}, $job->{job_name}, "RESUMING", "Job is resuming.");
            } elsif ($state eq "Running") {
                OAR::Modules::Judas::notify_user($dbh, $job->{notify}, $addr, $job->{job_user},
                    $job->{job_id}, $job->{job_name}, "RUNNING", "Job is running.");
            } elsif ($state eq "toLaunch") {
                update_current_scheduler_priority($dbh, $job->{job_id},
...
EOS
                ...
                if (defined($job->{assigned_moldable_job}) and
                    ($job->{assigned_moldable_job} ne "")) {

                    # Update last_job_date field for resources used
                    OAR::IO::update_scheduler_last_job_date($dbh, $date,
                        $job->{assigned_moldable_job});
                }

                if ($state eq "Terminated") {
                    OAR::Modules::Judas::notify_user($dbh, $job->{notify}, $addr, $job->{job_user},
                        $job->{job_id}, $job->{job_name}, "END", "Job stopped normally.");
                } else {
...
sub archive_some_moldable_job_nodes($$$) {
    ...
    if ($Db_type eq "Pg") {
        ...
        $dbh->do(
            "  UPDATE assigned_resources, resources
                    SET
...
sub resubmit_job($$;$) {
    ...
    return (-2)
      if (($job->{state} ne "Error") and
        ($job->{state} ne "Terminated") and
        ($job->{state} ne "Finishing"));
    return (-3) if (($lusr ne $job->{job_user}) and ($lusr ne "oar") and ($lusr ne "root"));

    lock_table($dbh, [ "challenges", "jobs" ]) if (not defined($nolock));

    # Verify the content of the ssh keys
    my ($job_challenge, $ssh_private_key, $ssh_public_key) =
      OAR::IO::get_job_challenge($dbh, $job_id);
    if (($ssh_public_key ne "") or ($ssh_private_key ne "")) {

        # Check if the keys are used by other jobs
        if (
            get_count_same_ssh_keys_current_jobs($dbh, $job->{job_user}, $ssh_private_key,
                $ssh_public_key) > 0
        ) {
            return (-4);
        }
...
sub frag_job($$) {
    ...
    if ((defined($job)) && (($lusr eq $job->{job_user}) or ($lusr eq "oar") or ($lusr eq "root"))) {
        ...
        if ($nbRes < 1) {
            my $date = get_date($dbh);
            $dbh->do(
                "INSERT INTO frag_jobs (frag_id_job,frag_date)
                      VALUES ($job_id,\'$date\')
                     ");
            add_new_event($dbh, "FRAG_JOB_REQUEST", $job_id,
                "User $lusr requested to frag the job $job_id");
            $result = 0;
        } else {
...
sub frag_inner_jobs($$$;$$) {
    ...
    if (lc(get_conf_with_default_param("KILL_INNER_JOBS_WITH_CONTAINER", "no")) eq "yes") {
        ...
        if (
            $dbh->do(
                "SELECT job_id FROM job_types WHERE job_id = $container_job_id AND type = 'container'"
            ) > 0
        ) {
            if (defined($message)) {
            ...
            $dbh->do(
                "INSERT INTO frag_jobs (frag_id_job, frag_date) SELECT job_id,\'$date\' FROM job_types WHERE type = \'inner=$container_job_id\' AND NOT EXISTS (SELECT * FROM frag_jobs WHERE frag_id_job = job_id)
                     ");
            $dbh->do(
                "INSERT INTO event_logs (type,job_id,date,description) SELECT \'FRAG_JOB_REQUEST\',t.job_id,\'$date\',\'Container job $container_job_id was fragged, frag inner job\' FROM job_types t WHERE t.type = \'inner=$container_job_id\' AND NOT EXISTS (SELECT * FROM event_logs e WHERE e.job_id = t.job_id AND e.type = \'FRAG_JOB_REQUEST\')
                     ");
        }
    }
}
...
sub ask_checkpoint_job($$) {
    ...
    if ((defined($job)) && (($lusr eq $job->{job_user}) or ($lusr eq "oar") or ($lusr eq "root"))) {
        if ($job->{state} eq "Running") {
            ...
            add_new_event($dbh, "CHECKPOINT", $job_id,
                "User $lusr requested a checkpoint on the job $job_id");
...
sub ask_signal_job($$$) {
    ...
    if ((defined($job)) && (($lusr eq $job->{job_user}) or ($lusr eq "oar") or ($lusr eq "root"))) {
        if ($job->{state} eq "Running") {
            ...
            #$dbh->do("LOCK TABLE event_log WRITE");
            add_new_event($dbh, "SIGNAL_$signal", $job_id,
                "User $lusr requested the signal $signal on the job $job_id");
...
sub hold_job($$$) {
    ...
    if (defined($job)) {
        if (defined($waiting_and_running) and
            (not $user_allowed_hold_resume) and
            ($lusr ne "oar") and
            ($lusr ne "root")) {
            return (-4);
        } elsif (($lusr eq $job->{job_user}) || ($lusr eq "oar") || ($lusr eq "root")) {
            if (($job->{'state'} eq "Waiting") or ($job->{'state'} eq "Resuming")) {
                add_new_event($dbh, $event_type, $job_id,
                    "User $lusr launched oarhold on the job $job_id");
                return 0;
            } elsif (
                (defined($waiting_and_running)) and
                (($job->{state} eq "toLaunch") or
    ...
    if (defined($job)) {
        if (($job->{'state'} eq "Suspended") and
            ...
            if (($job->{'state'} eq "Hold") or ($job->{'state'} eq "Suspended")) {
                add_new_event($dbh, "RESUME_JOB", $job_id,
                    "User $lusr launched oarresume on the job $job_id");
                return (0);
            }
            return (-3);
        }
...
sub job_refrag($$) {
    ...
    $dbh->do(
        "UPDATE frag_jobs SET frag_state = \'LEON\'
              WHERE frag_id_job = $job_id
             ");
}
...
sub job_leon_exterminate($$) {
    ...
    $dbh->do(
        "UPDATE frag_jobs SET frag_state = \'LEON_EXTERMINATE\'
              WHERE frag_id_job = $job_id
             ");
}
...
sub add_current_job_types($$$) {
    ...
    $dbh->do(
        "  INSERT INTO job_types (job_id,type,types_index)
...
EOS
    ...
    while (my $ref = $sth->fetchrow_hashref()) {
        ...
        $res->{$network_address}->{$job_id}->{types} =
          [ grep { defined($_) } @{ $res->{$network_address}->{$job_id}->{types} } ];
...
sub add_resource_job_pairs($$$) {
    ...
    my $query =
      "INSERT INTO assigned_resources (moldable_job_id,resource_id,assigned_resource_index) VALUES ";
...
sub get_jobs_for_user_query {
    ...
    if ($date_start ne "") {
        ...
        $second_query_date_start =
          " AND gantt_jobs_predictions_visu.start_time + moldable_job_descriptions.moldable_walltime >= $date_start ";
...
sub set_stagein($$$$$$) {
    ...
    $dbh->do(
        "INSERT INTO files (md5sum,location,method,compression,size)
...
sub add_admission_rule($$$$) {
    ...
    $dbh->do(
        "  INSERT INTO admission_rules (priority, enabled, rule)
    ...
    if ($sth->fetchrow_array() > 0) {
        $dbh->do(
            "  UPDATE admission_rules
                SET priority= $priority, enabled = " .
              ($enabled ? "'YES'" : "'NO'") . ", rule = $rule
                WHERE id=$id
               ");
        return ($id);
    } else {
...
sub add_resource($$$) {
    ...
    $dbh->do(
        "  INSERT INTO resources (network_address,state,state_num)
    ...
    my $date = get_date($dbh);
    $dbh->do(
        "  INSERT INTO resource_logs (resource_id,attribute,value,date_start)
...
sub add_resources($$) {
    ...
    my $query =
      "INSERT INTO resources (resource_id,state,state_num," . join(",", @properties) . ") VALUES ";
...
sub set_node_state($$$$$) {
    ...
    if ($state eq "Suspected") {
        if (
            ...
            OAR::Modules::Judas::oar_info(
                "NodeChangeState",
                "Try to turn the node $hostname into Suspected but it is not into the Alive state SO we do nothing\n",
                $session_id);
            return ();
        }
    } else {
    ...
    if ($Db_type eq "Pg") {
        ...
        $dbh->do(
            "  UPDATE resource_logs, resources
    ...
    foreach my $resource_id (keys %$resources_to_change) {
        if ($resources_info->{$resource_id}->{state} ne $resources_to_change->{$resource_id} or
            ...
            $insert_values =
              ...
              "', " . $date . ", '" . $resources_info->{$resource_id}{next_finaud_decision} . "'),";
            ...
            if ($resources_to_change->{$resource_id} eq 'Suspected') {
                push(@$resources_to_heal,
                    $resource_id . " " . $resources_info->{$resource_id}->{network_address});
            }
...
EOS
    ...
    if ($nbRowsAffected > 0) {
        ...
        if (defined($nbRowsAffected) and ($nbRowsAffected > 0)) {
            ...
            my $query =
              "INSERT INTO resource_logs (resource_id,attribute,value,date_start) VALUES ";
            foreach my $i (@ids) {
                $query .= " ($i, \'$property\', " .
                  (defined($value) ? "\'$value\'" : "\'<unset>\'") . ", \'$date\'),";
            }
            chop($query);
            $res = $dbh->do($query);
...
sub update_current_scheduler_priority($$$$$) {
    ...
    if (is_conf("SCHEDULER_PRIORITY_HIERARCHY_ORDER")) {
        ...
        if (((defined($types->{besteffort})) or (defined($types->{timesharing}))) and
            (
                (   $state eq "START" and
                    (is_an_event_exists($dbh, $job_id, "SCHEDULER_PRIORITY_UPDATED_START") <= 0)
                ) or
                (($state eq "STOP") and
                    (is_an_event_exists($dbh, $job_id, "SCHEDULER_PRIORITY_UPDATED_START") > 0)))
        ) {
...
EOS
            ...
            add_new_event($dbh, "SCHEDULER_PRIORITY_UPDATED_$state", $job_id,
                "Scheduler priority for job $job_id updated (" .
                  get_conf("SCHEDULER_PRIORITY_HIERARCHY_ORDER") . ")");
        }
    }
}
...
sub create_a_queue($$$$) {
    ...
    $dbh->do(
        "  INSERT INTO queues (queue_name,priority,scheduler_policy)
...
sub add_gantt_scheduled_jobs($$$$) {
    ...
    $dbh->do(
        "INSERT INTO gantt_jobs_predictions (moldable_job_id,start_time)
    ...
    chop($str);
    $dbh->do(
        "INSERT INTO gantt_jobs_resources (moldable_job_id,resource_id)
              VALUES $str
             ");
}
...
sub remove_gantt_resource_job($$$) {
    ...
    $dbh->do(
        "DELETE FROM gantt_jobs_resources WHERE moldable_job_id = $job AND resource_id = $resource"
    );
}
...
sub update_gantt_visualization($) {
    ...
    lock_table(
        $dbh,
        [   "gantt_jobs_predictions_visu", "gantt_jobs_resources_visu",
...
##    $dbh->do("OPTIMIZE TABLE ganttJobsResources_visu, ganttJobsPredictions_visu");
    $dbh->do(
        "INSERT INTO gantt_jobs_predictions_visu
              SELECT *
...
sub gantt_flush_tables($$$) {
    ...
    if (defined($log)) {
        ...
        $dbh->do(
            "  INSERT INTO gantt_jobs_predictions_log (sched_date,moldable_job_id,start_time)
        ...
        ");
        $dbh->do(
            "  INSERT INTO gantt_jobs_resources_log (sched_date,moldable_job_id,resource_id)
    ...
    if ($#moldable_jobs_to_keep >= 0) {
        ...
        $dbh->do(
            "  DELETE FROM gantt_jobs_predictions
                    WHERE
                        $sql
                 ");
        $dbh->do(
            "  DELETE FROM gantt_jobs_resources
                    WHERE
                        $sql
                 ");
    } else {
...
EOS
    ...
    while (my @ref = $sth->fetchrow_array()) {
        ...
        if ($start eq 0 || $stop <= $start || $ref[8] < 1) {
            ...
            update_accounting(
                $dbh,    $sth1,   $start,  $stop,  $windowSize,
                $ref[4], $ref[7], $ref[5], "USED", $ref[6]);
            update_accounting(
                $dbh,        $sth1,   $start,  $theoricalStopTime,
                $windowSize, $ref[4], $ref[7], $ref[5],
                "ASKED",     $ref[6]);
            $dbh->do(
                "  UPDATE jobs
                        SET accounted = \'YES\'
...
sub update_accounting($$$$$$$$$$) {
    ...
    while ($stop > $start) {
        ...
        add_accounting_row($dbh, $sth1, $windowStart, $windowStop, $user, $project, $queue, $type,
            $conso);
...
sub add_accounting_row($$$$$$$$$) {
    ...
    if (defined($ref[0])) {
        ...
        $conso += $ref[0];
        print(
            "[ACCOUNTING] Update the existing window $start --> $stop , project $project, user $user, queue $queue, type $type with conso = $conso s\n"
        );
        $dbh->do(
            "  UPDATE accounting
        ...
        # Create the window
        print(
            "[ACCOUNTING] Create new window $start --> $stop , project $project, user $user, queue $queue, type $type with conso = $conso s\n"
        );
        $dbh->do(
            "  INSERT INTO accounting (accounting_user,consumption_type,queue_name,window_start,window_stop,consumption,accounting_project)
...
sub add_new_event($$$$) {
    ...
    my $date = get_date($dbh);
    $dbh->do(
        "INSERT INTO event_logs (type,job_id,date,description) VALUES (\'$type\',$job_id,\'$date\',$description)"
    );
}
...
sub add_new_event_with_host($$$$$) {
    ...
    $dbh->do(
        "  INSERT INTO event_logs (type,job_id,date,description)
    ...
    foreach my $n (@{$hostnames}) {
        if (!defined($tmp{$n})) {
            $dbh->do(
                "  INSERT INTO event_log_hostnames (event_id,hostname)
                        VALUES ($event_id,\'$n\')
                     ");
            $tmp{$n} = 1;
        }
...
sub add_walltime_change_request($$$$$$$) {
    ...
    my $timeout         = shift;
    $dbh->do(
        "INSERT INTO walltime_change (job_id,pending,force,delay_next_jobs,whole,timeout) VALUES ($job_id,$pending,'$force','$delay_next_jobs','$whole','$timeout')"
    );
}
...
sub update_walltime_change_request($$$$$$$$$$$) {
    ...
    my $timeout                      = shift;
    $dbh->do(
        "UPDATE walltime_change SET pending=$pending" .
          ...
          ( (defined($granted_with_delay_next_jobs)) ?
...
sub get_walltime_change_for_job($$) {
    ...
    my $job_id = shift;
    my $sth    = $dbh->prepare(
        "SELECT pending, force, delay_next_jobs, whole, granted, granted_with_force, granted_with_delay_next_jobs, granted_with_whole, timeout FROM walltime_change WHERE job_id = $job_id"
    );
    $sth->execute();
    my $ref = $sth->fetchrow_hashref();
    return $ref;
...
# the scheduling for other users. The user can always delete his job if needed for extratime.
    ...
    if (defined($job_types->{timesharing})) {
        if ($job_types->{timesharing} eq 'user,*' or $job_types->{timesharing} eq '*,user') {
            $exclude .=
              "((t.type = 'timesharing=user,*' OR t.type = 'timesharing=*,user') and j.job_user = $job_user) OR ";
        } elsif ($job_types->{timesharing} eq 'name,*' or $job_types->{timesharing} eq '*,name') {
            $exclude .=
              "((t.type = 'timesharing=*,name' OR t.type = 'timesharing=name,*') and j.job_name = $job_name) OR ";
        } elsif ($job_types->{timesharing} eq 'name,user' or
            $job_types->{timesharing} eq 'user,name') {
            $exclude .=
              "((t.type = 'timesharing=user,name' OR t.type = 'timesharing=name,user') and j.job_name = '$job_name' AND j.job_user = '$job_user') OR ";
...
#lock_table($base,["jobs","job_state_logs","resources","assigned_resources","resource_state_logs","event_logs","challenges","moldable_job_descriptions","job_types","job_dependencies","job_resource_groups","job_resource_descriptions"]);
    ...
    if (($refJob->{'state'} eq "Running") or
        ($refJob->{'state'} eq "Launching") or
        ($refJob->{'state'} eq "Suspended") or
        ($refJob->{'state'} eq "Resuming")) {
        ...
        if ($error == 0) {
            ...
            push(
                ...
                {   type   => "SWITCH_INTO_TERMINATE_STATE",
                    string => "[bipbip $job_id] Ask to change the job state"
                });
            job_finishing_sequence($base, $server_epilogue_script, $remote_host, $remote_port,
                $job_id, \@events, $session_id);
            ...
            if ((defined($types->{besteffort})) and (defined($types->{idempotent}))) {
                if (OAR::IO::is_an_event_exists($base, $job_id, "BESTEFFORT_KILL") > 0) {
                    ...
                    oar_warn(
                        "Bipbip",
                        "We resubmit the job $job_id (new id = $new_job_id) because it is a besteffort and idempotent job.\n",
                        $Session_id);
                    push(
                        @events,
                        {   type   => "RESUBMIT_JOB_AUTOMATICALLY",
                            string =>
                              "[bipbip $job_id] the job $job_id is a besteffort and idempotent job so we resubmit it (new id = $new_job_id)"
                        });
                }
            }
            job_finishing_sequence($base, $server_epilogue_script, $remote_host, $remote_port,
            ...
            my $strWARN =
              "[bipbip $job_id] The job $job_id was killed by Leon and oarexec epilogue was in error";
            my $types = OAR::IO::get_job_types_hash($base, $job_id);
            if ((defined($types->{besteffort})) and (defined($types->{idempotent}))) {
                if (OAR::IO::is_an_event_exists($base, $job_id, "BESTEFFORT_KILL") > 0) {
                    my $new_job_id = OAR::IO::resubmit_job($base, $job_id);
            ...
            my $strWARN =
              "[bipbip $job_id] Cannot get shell of user $user, so I suspect node $hosts->[0]";
            ...
            my $strWARN = "[bipbip $job_id] oarexec cannot create tmp directory on $hosts->[0]: " .
              OAR::Tools::get_default_oarexec_directory();
            ...
            my $strWARN =
              "[bipbip $job_id] Cannot go into the working directory $launchingDirectory of the job on node $hosts->[0]";
            ...
            my $strWARN =
              "[bipbip $job_id] Cannot create .stdout and .stderr files in $launchingDirectory on the node $hosts->[0]";
            ...
            my $log_strWARN =
              "Cannot go into the working directory $launchingDirectory of the job on node $hosts->[0] AND epilogue is in error";
            my $strWARN = "[bipbip $job_id] $log_strWARN";
            oar_warn("Bipbip", "$log_strWARN\n", $session_id, $job_id);
            push(@events, { type => "WORKING_DIRECTORY", string => $strWARN });
            push(@events, { type => "EPILOGUE_ERROR",    string => $strWARN });
            job_finishing_sequence($base, $server_epilogue_script, $remote_host, $remote_port,
                $job_id, \@events, $session_id);
            ...
            my $log_strWARN = "Cannot create STDOUT and STDERR files AND epilogue is in error";
            my $strWARN     = "[bipbip $job_id] $log_strWARN";
            oar_warn("Bipbip", "$log_strWARN\n", $session_id, $job_id);
            push(@events, { type => "OUTPUT_FILES",   string => $strWARN });
            push(@events, { type => "EPILOGUE_ERROR", string => $strWARN });
            ...
            my $strWARN =
              "[bipbip $job_id] oarexec received a SIGUSR1 signal and there was an epilogue error";
            ...
            my $strWARN =
              "[bipbip $job_id] launching oarexec timeout, exit value = $error; the job $job_id is in Error and the node $hosts->[0] is Suspected";
...
#            }
            ...
            my $strWARN =
              "[bipbip $job_id] error of oarexec, exit value = $error; the job $job_id is in Error";
            push(@events, { type => "EXIT_VALUE_OAREXEC", string => $strWARN });
            job_finishing_sequence($base, $server_epilogue_script, $remote_host, $remote_port,
                $job_id, \@events, $session_id);
        }
    } else {
        OAR::Modules::Judas::oar_info("Bipbip",
            "I was previously killed or Terminated but I did not know that!!\n",
            $session_id, $job_id);
        unlock_table($base);
    }
...
sub job_finishing_sequence($$$$$$$) {
    my ($dbh, $epilogue_script, $almighty_host, $almighty_port, $job_id, $events, $session_id) = @_;
    ...
    if (defined($epilogue_script)) {
        ...
        if ($@) {
            ...
            my $str =
              "[JOB FINISHING SEQUENCE] Server epilogue exit code $exit_value (!=0) (cmd: $cmd)";
            oar_error("JobFinishingSequence",
                "Server epilogue exit code $exit_value (!=0) (cmd: $cmd)\n", $session_id);
            push(@{$events}, { type => "SERVER_EPILOGUE_EXIT_CODE_ERROR", string => $str });
        }
    }

    my $types = OAR::IO::get_job_types_hash($dbh, $job_id);
    if ((!defined($types->{deploy})) and
        ...
        if (defined($cpuset_field)) {
            ...
            my $job          = get_job($dbh, $job_id);
            my $cpuset_nodes = OAR::IO::get_cpuset_values_for_a_moldable_job($dbh, $cpuset_field,
                $job->{assigned_moldable_job});
            if (defined($cpuset_nodes) and (keys(%{$cpuset_nodes}) > 0)) {
                OAR::Modules::Judas::oar_info("JobFinishingSequence",
                    "CPUSET Clean cpuset on each nodes\n",
                    $session_id, $job_id);
                my $taktuk_cmd = get_conf("TAKTUK_CMD");
                my $job_user   = $job->{job_user};
                my ($job_challenge, $ssh_private_key, $ssh_public_key) =
                  OAR::IO::get_job_challenge($dbh, $job_id);
                $ssh_public_key = OAR::Tools::format_ssh_pub_key($ssh_public_key, $cpuset_full_path,
                    $job->{job_user}, $job_user);
                ...
                my $cpuset_data_hash = {
                    ...
                    compute_thread_siblings =>
                      get_conf_with_default_param("COMPUTE_THREAD_SIBLINGS", "no"),
                    ssh_keys => {
                        public => {
                    ...
                    stdout_file                         =>
                      OAR::Tools::replace_jobid_tag_in_string($job->{stdout_file}, $job_id),
                    stderr_file =>
                      OAR::Tools::replace_jobid_tag_in_string($job->{stderr_file}, $job_id),
                ...
                my ($tag, @bad) = manage_remote_commands(
                ...
                if ($tag == 0) {
                    my $str =
                      "[JOB FINISHING SEQUENCE] [CPUSET] [$job_id] Bad cpuset file: $cpuset_file\n";
                    oar_error("JobFinishingSequence", "Bad cpuset file: $cpuset_file\n",
                        $session_id, $job_id);
                    push(@{$events}, { type => "CPUSET_MANAGER_FILE", string => $str });
                } elsif ($#bad >= 0) {
                    oar_error("JobFinishingSequence",
                        "Cpuset error and register event CPUSET_CLEAN_ERROR on nodes: @bad\n",
                        $session_id, $job_id);
                    push(
                        @{$events},
                        {   type   => "CPUSET_CLEAN_ERROR",
                            string =>
                              "[job_finishing_sequence] OAR suspects nodes for the job $job_id: @bad",
                            hosts => \@bad
                        });
                }
            }
    ...
    if ((is_conf("ACTIVATE_PINGCHECKER_AT_JOB_END")) and
        (lc(get_conf("ACTIVATE_PINGCHECKER_AT_JOB_END")) eq "yes") and
        ...
        if ($#bad_pingchecker >= 0) {
            oar_error(
                "JobFinishingSequence",
                "PING_CHECKER_NODE_SUSPECTED_END_JOB OAR suspects nodes for the job $job_id: @bad_pingchecker\n",
                $session_id,
                $job_id);
            push(
                @{$events},
                {   type   => "PING_CHECKER_NODE_SUSPECTED_END_JOB",
                    string =>
                      "[job_finishing_sequence] OAR suspects nodes for the job $job_id: @bad_pingchecker",
                    hosts => \@bad_pingchecker
                });
        }
    }
    #
...
sub manage_remote_commands($$$$$$$$$) {
    ...
    if (!defined($taktuk_cmd)) {
        ...
        if ($pid == 0) {
            ...
            $SIG{USR2} = 'DEFAULT';
            my $cmd =
              "$taktuk_cmd -c '$ssh_cmd' " . '-o status=\'"STATUS $host $line\n"\'' . " -f '<&=" .
              fileno(tak_node_read) .
              "' broadcast exec [ perl - $action ], broadcast input file [ - ], broadcast input close";
