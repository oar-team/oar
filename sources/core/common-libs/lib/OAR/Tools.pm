# $Id$
{
    ...
    sub format_ssh_pub_key($$$$) {
        ...
        return ('environment="OAR_CPUSET=' .
              $cpuset . '",environment="OAR_JOB_USER=' . $job_user . '" ' . $key . "\n");
    }
    ...
    sub notify_tcp_socket($$$) {
        ...
        my $socket = IO::Socket::INET->new(
            PeerAddr => $almighty_host,
            PeerPort => $almighty_port,
            Proto    => "tcp",
            Type     => SOCK_STREAM
          ) or
          return ("Could not connect to the socket $almighty_host:$almighty_port");
...
#my $cmd = "$ssh_cmd -x -T $host bash -c \"test -e $file && PROC=\\\$(cat $file) && kill -s CONT \\\$PROC && kill -s $signal \\\$PROC\"";
        ...
        if (defined($user_signal) && $user_signal ne '') {
            ...
            $cmd[$c] =
              "bash -c 'echo $user_signal > $signal_file && test -e $file && PROC=\$(cat $file) && kill -s CONT \$PROC && kill -s $signal \$PROC'";
            $c++;
        } else {
            $cmd[$c] =
              "bash -c 'test -e $file && PROC=\$(cat $file) && kill -s CONT \$PROC && kill -s $signal \$PROC'";
            $c++;
        }
        $SIG{PIPE} = 'IGNORE';
        my $pid = fork();
    ...
    sub get_oarexecuser_perl_script_for_oarexec($$$$$$$$$$$$$$$@) {
        my ($node_file,    $job_id,           $array_id,            $array_index,
            $user,         $shell,            $launching_directory, $stdout_file,
            $stderr_file,  $resource_file,    $job_name,            $job_project,
            $job_walltime, $job_walltime_sec, $job_env,             @cmd
        ) = @_;
...
';
    ...
    sub sentinelle($$$$$) {
        ...
        while (($index <= $#$nodes) or ($#timeout >= 0)) {
            ...
            while (
                ...
                (($timeout[$t]->[1] <= time()) or
                    (!defined($running_processes{ $timeout[$t]->[0] })))
            ) {
    ...
    sub check_resource_property($) {
        ...
        if ($prop =~
            /^(resource_id|network_address|state|state_num|next_state|finaud_decision|next_finaud_decision|besteffort|desktop_computing|deploy|expiry_date|last_job_date|available_upto|walltime|nodes|type|suspended_jobs|scheduler_priority)$/
        ) {
            return (1);
        } else {
            return (0);
    ...
    sub check_resource_system_property($) {
        ...
        if ($prop =~
            /^(resource_id|state|state_num|next_state|finaud_decision|next_finaud_decision|last_job_date|suspended_jobs|expiry_date|scheduler_priority)$/
        ) {
            return (1);
        } else {
            return (0);
