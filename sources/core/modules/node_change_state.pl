            ...
            if (defined($jobtypes->{noop})){
                ...
                if (defined($Cpuset_field)){
                    ...
                    if (defined($cpuset_nodes)){
                        ...
                        my ($tag,@bad) = OAR::Tools::manage_remote_commands([keys(%{$cpuset_nodes})],$suspend_data_hash,$suspend_file,"suspend",$openssh_cmd,$taktuk_cmd,$base,$Module_name,$Session_id);
                        if ($tag == 0){
