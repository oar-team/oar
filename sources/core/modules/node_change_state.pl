...
OAR::IO::lock_table(
    ...
    [   "resources",                 "assigned_resources",
        "jobs",                      "job_state_logs",
        "event_logs",                "event_log_hostnames",
        "frag_jobs",                 "moldable_job_descriptions",
        "challenges",                "job_types",
        "job_dependencies",          "job_resource_groups",
        "job_resource_descriptions", "resource_logs"
    ]);
...
foreach my $i (@events_to_check) {
    ...
    ####################################################
    if ((($i->{type} eq "SWITCH_INTO_TERMINATE_STATE") or ($i->{type} eq "SWITCH_INTO_ERROR_STATE"))
        and
        (defined($job->{exit_code}) and (($job->{exit_code} >> 8) == 99))) {
        if ((defined($jobtypes->{idempotent}))) {
            if (($job->{reservation} eq "None") and
                ($job->{job_type} eq "PASSIVE")                                          and
                (OAR::IO::is_job_already_resubmitted($base, $i->{job_id}) == 0)          and
                (OAR::IO::is_an_event_exists($base, $i->{job_id}, "SEND_KILL_JOB") <= 0) and
                ($job->{stop_time} - $job->{start_time} > 60)) {
                my $new_job_id = OAR::IO::resubmit_job($base, $i->{job_id}, 1);
                my $log_msg =
                  "Resubmiting job $i->{job_id} => $new_job_id (type idempotent & exit code = 99 & duration > 60s)";
    ...
    if (($i->{type} eq "PING_CHECKER_NODE_SUSPECTED") ||
        ...
        if (($job->{reservation} eq "None") or
            ($i->{type} eq "RESERVATION_NO_NODE") or
            ($job->{assigned_moldable_job} == 0)) {
            OAR::IO::set_job_state($base, $i->{job_id}, "Error");
        } elsif ($job->{reservation} ne "None") {
            if (($i->{type} ne "PING_CHECKER_NODE_SUSPECTED") &&
    ...
    if (($i->{type} eq "CPUSET_CLEAN_ERROR") ||
  ...
  # Restrict Suspected state to the first node (node really connected with OAR) for some event types
        if (($i->{type} eq "PING_CHECKER_NODE_SUSPECTED") or
            ...
            oar_info($Module_name,
                "Not suspecting any nodes because error is on the deploy or cosystem frontend\n",
                $Session_id);
        } else {
        ...
        if ($#hosts >= 0) {
            ...
            my $log_msg =
              "Set nodes to suspected after error ($i->{type}): " . join(", ", @hosts) . "\n";
            my $msg = "[NodeChangeState] $log_msg";
            oar_warn($Module_name, $log_msg, $Session_id);
    ...
    ########################################
    if (($i->{type} eq "SERVER_PROLOGUE_TIMEOUT") ||
        ...
        oar_error($Module_name,
            "Server admin script error ($i->{type}), stopping all scheduler queues\n",
            $Session_id, $i->{job_id});
        send_log_by_email(
            "Stop all scheduling queues",
            "[NodeChangeState] Server admin script error (job $i->{job_id}), stopping all scheduler queues: $i->{type}. Fix errors and run `oarnotify -E' to re-enable them.\n"
        );
...
# Do not resubmit an advance reservation or an interactive job because this makes no sense.
        if (not defined($jobtypes->{deploy}) and
            not defined($jobtypes->{cosystem}) and
            ($job->{reservation} eq "None")    and
            ($job->{job_type} eq "PASSIVE")    and
            (OAR::IO::is_job_already_resubmitted($base, $i->{job_id}) == 0)) {
            my $new_job_id = OAR::IO::resubmit_job($base, $i->{job_id}, 1);
            my $log_msg =
              "Resubmiting job $i->{job_id} => $new_job_id) (due to event $i->{type})\n";
            my $msg = "[NodeChangeState] $log_msg";
    ...
    ####################################
    if (($i->{type} eq "HOLD_WAITING_JOB") ||
        ($i->{type} eq "HOLD_RUNNING_JOB") ||
        ($i->{type} eq "RESUME_JOB")) {
        if ((($i->{type} eq "HOLD_WAITING_JOB") or ($i->{type} eq "HOLD_RUNNING_JOB")) and
        ...
        } elsif ((($i->{type} eq "HOLD_WAITING_JOB") or ($i->{type} eq "HOLD_RUNNING_JOB")) and
            ...
            if (defined($jobtypes->{noop})) {
                ...
                if (defined($Cpuset_field)) {
                    ...
                    if (defined($cpuset_nodes)) {
                        ...
                        $openssh_cmd = OAR::Tools::get_default_openssh_cmd()
                        ...
                        if ($tag == 0) {
                            my $log_str =
                              "[SUSPEND_RESUME] [$i->{job_id}] bad suspend/resume file: $suspend_file\n";
                            my $str = "[NodeChangeState] $log_str";
                            oar_error($Module_name, $log_str, $Session_id, $i->{job_id});
                            OAR::IO::add_new_event($base, "SUSPEND_RESUME_MANAGER_FILE",
                                $i->{job_id}, $str);
                        } else {
                            if (($#bad < 0)) {
                                OAR::IO::suspend_job_action($base, $i->{job_id},
                                    $job->{assigned_moldable_job});
                                ...
                                if (defined($suspend_script)) {
                                    ...
                                    eval {
                                        ...
                                        oar_info(
                                            $Module_name,
                                            "Launching post suspend script: $suspend_script $i->{job_id}\n",
                                            $Session_id,
                                            $i->{job_id});
                                    ...
                                    if ($@ || ($script_error != 0)) {
                                        my $log_str =
                                          "Suspend script error, resuming job: $@; return code = $script_error\n";
                                        ...
                                        OAR::IO::add_new_event($base, "SUSPEND_SCRIPT_ERROR",
                                            $i->{job_id}, $str);
                                        OAR::IO::set_job_state($base, $i->{job_id}, "Resuming");
                                        OAR::Tools::notify_tcp_socket($Remote_host, $Remote_port,
                                            "Qresume");
                                    }
                                }
                            } else {
                                ...
                                my $str =
                                  "[NodeChangeState] [SUSPEND_RESUME] [$i->{job_id}] error on several nodes: @bad\n";
                                oar_error($Module_name, $str, $Session_id, $i->{job_id});
                                OAR::IO::add_new_event_with_host($base, "SUSPEND_ERROR",
                                    $i->{job_id}, $str, \@bad);
    ...
    ####################################
    if (($i->{type} eq "FRAG_JOB_REQUEST")) {
        my ($addr, $port) = split(/:/, $job->{info_type});
        OAR::Modules::Judas::notify_user($base, $job->{notify}, $addr, $job->{job_user},
            $job->{job_id}, $job->{job_name}, "INFO",
            "Your job was asked to be deleted - $i->{description}");
    }
...
foreach my $h (keys(%debug_info)) {
    my $log_str = "state change requested for $h: " .
      join(" ", map { "$_:$debug_info{$h}->{$_}" } keys(%{ $debug_info{$h} })) . "\n";
    my $str = "[NodeChangeState] $log_str";
...
if (defined($Healing_exec_file) && @resources_to_heal > 0) {
    ...
    if (
        !defined(
            OAR::Tools::fork_and_feed_stdin($Healing_exec_file, $timeout_cmd, \@resources_to_heal))
    ) {
        oar_error(
            $Module_name,
            "Try to launch the command $Healing_exec_file to heal resources, but the command timed out($timeout_cmd s).\n",
            $Session_id);
    }
}
