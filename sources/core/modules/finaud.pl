...
oar_debug($Module_name, "Finaud started\n", $Session_id);

oar_debug($Module_name, "Check Alive and Suspected nodes\n", $Session_id);
...
oar_debug($Module_name, "Testing resource(s) on: @Nodes_to_check\n", $Session_id);
...
foreach my $i (values(%Nodes_hash)){
    if (defined($bad_node_hash{$i->{network_address}}) and ($i->{state} eq "Alive")){
        ...
        if ($rows > 0) {
            ...
            oar_debug($Module_name, "Detected an error on $i->{network_address}, set next state to Suspected\n", $Session_id);
            $return_value = 1;
        } else {
            oar_debug($Module_name, "Detected an error on $i->{network_address}, but do nothing because some changes happened in our back\n", $Session_id);
        }
        ...
        oar_debug($Module_name, "Set the next state of $i->{network_address} to Alive\n", $Session_id);
        $return_value = 1;
    }
}
...
oar_debug($Module_name, "Finaud ended: $return_value\n", $Session_id);
