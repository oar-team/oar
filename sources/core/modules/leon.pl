...
if (defined($ARGV[0])) {
    ...
    if (defined($frag_state) and ($frag_state eq "LEON_EXTERMINATE")) {
        ...
        OAR::IO::job_finishing_sequence($base, $Server_epilogue, $Server_hostname, $Server_port,
            $job_id, \@events, $Session_id);
        OAR::Tools::notify_tcp_socket($Server_hostname, $Server_port, "ChState");
    } else {
        oar_error($Module_name,
            "Leon was called on job but its frag_state is not LEON_EXTERMINATE\n",
            $Session_id, $job_id);
    }
...
OAR::IO::lock_table(
    ...
    [   "jobs",                      "job_state_logs",
        ...
        "frag_jobs",                 "event_logs",
        "moldable_job_descriptions", "job_types",
        "job_resource_descriptions", "job_resource_groups",
        "challenges",                "job_dependencies",
    ...
    if (($j->{state} eq "Waiting") || ($j->{state} eq "Hold")) {
        ...
        if ($j->{job_type} eq "INTERACTIVE") {
            ...
            if (!defined(OAR::Tools::notify_tcp_socket($addr, $port, "JOB KILLED"))) {
                ...
                oar_info(
                    $Module_name,
                    "Cannot open connection to oarsub client for job, which is normal if user typed Ctrl-C\n",
                    $Session_id,
                    $j->{job_id});
            }
            OAR::Tools::inhibit_notify_tcp_socket();
        ...
        $Exit_code = 1;
    } elsif (($j->{state} eq "Terminated") ||
        ($j->{state} eq "Error") ||
        ($j->{state} eq "Finishing")) {
        ...
        if (defined($types->{noop})) {
            ...
            OAR::IO::job_finishing_sequence($base, $Server_epilogue, $Server_hostname,
                $Server_port, $j->{job_id}, [], $Session_id);
            $Exit_code = 1;
        } else {
            ...
            if (defined($host_to_connect_via_ssh)) {
                oar_info($Module_name,
                    "Kill -TERM job (oarexec) on host $host_to_connect_via_ssh\n",
                    $Session_id, $j->{job_id});
                OAR::IO::add_new_event($base, "SEND_KILL_JOB", $j->{job_id},
                    "[Leon] Send the kill TERM signal to oarexec on $host_to_connect_via_ssh for job $j->{job_id}"
                );
                OAR::Tools::signal_oarexec($host_to_connect_via_ssh, $j->{job_id}, "TERM", 0,
                    $base, $Openssh_cmd, '');
            } else {
