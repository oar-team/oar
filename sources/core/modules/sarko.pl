...
oar_debug($Module_name, "JOBDEL_SOFTWALLTIME = $Leon_soft_walltime; JOBDEL_WALLTIME = $Leon_walltime\n", $Session_id);
...
oar_debug($Module_name, "Hello, identity control !!!\n", $Session_id);
...
foreach my $j (OAR::IO::get_timered_job($base)){
    ...
    if (($job_ref->{state} eq "Terminated") || ($job_ref->{state} eq "Error") || ($job_ref->{state} eq "Finishing")){
        oar_debug($Module_name, "Set to FRAGGED job $j->{job_id}\n", $Session_id);
        ...
        oar_debug($Module_name, "frag date for job $j->{job_id}: $frag_date\n", $Session_id);
        if (($current > $frag_date+$Leon_soft_walltime) && ($current <= $frag_date+$Leon_walltime)){
            oar_debug($Module_name, "Leon will RE-FRAG bipbip of job $j->{job_id}\n", $Session_id);
            OAR::IO::job_refrag($base,$j->{job_id});
            $guilty_found=1;
        }elsif ($current > $frag_date+$Leon_walltime){
            oar_debug($Module_name, "Leon will EXTERMINATE bipbip of job $j->{job_id}\n", $Session_id);
            OAR::IO::job_leon_exterminate($base,$j->{job_id});
            $guilty_found=1;
        }else{
            oar_debug($Module_name, "The leon timer is not yet expired for the job $j->{job_id}; I do nothing\n", $Session_id);
        }
    }
}
...
foreach my $job (OAR::IO::get_jobs_in_state($base, "Running")){
    ...
    oar_debug($Module_name, "From $start with $max; current time=$current\n", $Session_id, $job->{job_id});
    if ($current > $start+$max){
        oar_debug($Module_name, "--> (Elapsed)\n", $Session_id);
        $guilty_found=1;
        ...
        oar_debug($Module_name, "Send checkpoint signal to the job $job->{job_id}\n", $Session_id);
        ...
        if ($@){
            ...
            if ($exit_codes[0] == 0){
                ...
                oar_debug($Module_name, "$str_comment\n", $Session_id);
...
if ($dead_switch_time > 0){
    ...
    foreach my $r (OAR::IO::get_absent_suspected_resources_for_a_timeout($base,$dead_switch_time)){
        ...
        oar_debug($Module_name, "Set the next state of $r to Dead\n", $Session_id);
        $notify = 1;
    }
    if ($notify > 0){
        my $remote_host = get_conf("SERVER_HOSTNAME");
