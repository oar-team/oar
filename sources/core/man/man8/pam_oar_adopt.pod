...
Follows an example of configuration of PAM with B<pam_oar_adopt> on Debian-like systems:
...
Conversly, in an installation where either of OAR's deploy or cosystem job types is used, which requires the oar-node package to also be installed on the deploy or cosystem frontend, it is usually normal to let any user ssh to that frontend regardless of jobs. As a result on such a frontend, B<pam_oar_adopt> should not be installed (on Debian-like systems, one may use B<pam-auth-update> to deactivate the oar-node PAM profile).
