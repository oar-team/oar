...
Request walltime increase to be trialed or applied at once.
...
Specify a timeout after which the walltime change request will be aborted if not already accepted by the scheduler. By default, there is no timeout unless a default timeout is set by the administrator.
