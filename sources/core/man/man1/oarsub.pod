...
Notes:
  - a job with the type B<besteffort> will be scheduled with the lowest priority and will be killed if a "normal" job needs its resources.
  ...
  - a job with the type B<noop> does nothing except reserving the resources. It is ended at the end of it walltime or when using the oardel command.
