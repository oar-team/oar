...
B<oarsub> [OPTIONS] -r "<START DATE>[, <END DATE>]"
...
A batch job will be scheduled by OAR when the requested resources are available, with no time period constraint. In interactive mode (B<-I>), B<oarsub> will wait until the scheduler can give the resources and connect to the job.

=head2 ADVANCE RESERVATION JOBS

An advance reservation allow to request OAR that the job starts (and optionally ends) at a given time instead of letting the scheduler decide, and waits for validation. OAR will respond with I<OK> or I<KO> if it can (or not) schedule the job (if the requested resources are available at given time).
...
=item B<-r, --reservation> "<START DATE>[, <END DATE>]"

Request that the job starts (optionally ends) at a specified time. A job creation using this option is called an I<advance reservation> (as opposite to a I<submission>). Start and end dates can be given in the form I<YYYY-MM-DD hh:mm:ss>, but if I<YYYY-MM-DD> is omitted, it defaults to the current day. In hh:mm::ss, ss defaults to I<00> if omitted, as well as I<mm> if omitted also. Quotes are only required when spaces are present. When end date is provided, a walltime is calculated and will only be used if not provided in the resources request. The special keyword B<now> can also be use to request an advance reservation job that actually starts just now.
