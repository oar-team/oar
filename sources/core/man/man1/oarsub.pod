...
=item B<-a, --after> <OAR JOB ID>[,min[,max]]

Add a dependency to a given job, with optional min and max relative start time constraints.
Min and max are of the form [+T or [-T or ]+T or ]-T, with ] and [ refering to the start or the end time of the dependency job, and T a time in seconds.   
...
Import the job-key to use inline (text), instead of generating a new one.

=item B<-e, --export-job-key-to-file>

Export the job key to a file.
