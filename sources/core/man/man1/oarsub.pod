...
The OAR scheduler is in charge of providing a set of resources which matches the B<oarsub> resource request (see the I<-l> option). Once scheduled and launched, the job executes the command on the first node (machine) of the assigned the resources, with some environment variables set (e.g. the assigned resources list, the job duration a.k.a walltime, ...). If many nodes (machines) are assigned to the job, it is let to the user's command to dispatch an execution on all nodes.
