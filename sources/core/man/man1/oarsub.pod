...
Specify a specific type (I<besteffort>, I<cosystem>, I<checkpoint>, I<timesharing>, I<idempotent>)

Note: a job with a type of B<idempotent> will be automatically resubmitted if its exit code is 99.
...
Specify the files that will store the standard output stream of the job. The %jobid% pattern is automatically replaced.

=item B<-E> <FILE>

Specify the files that will store the standard error stream of the job. The %jobid% pattern is automatically replaced.
