...
If resources are already registered, offsets can be provided, or guessed from the database.

This script is also a good example of how one could create resources by himself using shell loops and oarnodesetting.
...
First host number to use

=item B<--cpu0> <#>

First cpu number to use

=item B<--core0> <#>

First core number to use

=item B<--thread0> <#>

First thread number to use

=item B<--cpuset> <#>

# of cpusets on host (default=cpus*cores*threads)

=item B<-p>, B<--other-properties> <str>

Extra properties for every resources
...
Guess first host/cpu/core/thread number to use from the OAR database (unless host0/cpu0/core0/thread0 is set). Warning: host, cpu, core and thread properties must exist in your database or you will get errors.
...
$ oar_resources_add --hosts=2 --host0 5 --cpu0 8 --core0=64 --thread0=128 -p "mem=1024 gpu=NO"
...
=head1 COPYRIGHTS
 ...
 Copyright 2004-2013 Laboratoire d'Informatique de Grenoble (http://www.liglab.fr). This software is licensed under the GNU Library General Public License. There is NO warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
