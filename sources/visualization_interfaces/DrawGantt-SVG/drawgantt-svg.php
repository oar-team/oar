	...
	function svg_jobs($y) {
		...
		foreach ($this->job_resource_id_groups as $grp) {
			...
			if ($width > $CONF['gantt_min_job_width_for_label']) {
				$labels_output .= '<text font-size="10" x="'.(date2px($grp->job->start_time) + $width / 2).'" y="'.($y + ($grp->size() + 1) * $CONF['scale'] / 2).'" text-anchor="middle" onmouseover="mouseOver(evt,\''.$grp->job->svg_text().'\')" onmouseout="mouseOut(evt)" onmousemove="mouseMove(evt)">';
				$labels_output .= $grp->job->job_id;
				$labels_output .= '</text>';
			}
		}
...
//$link = mysql_connect("localhost", 'oarreader', 'read')
