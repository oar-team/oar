    ...
    for r in node_resources:
        ...
        if r["state"] == "Dead":
            ...
            try: 
                ...
                if r["drain"] == "YES":
                    cprint (Back.YELLOW+Fore.BLACK+"d")
                else:
...
cprint(Back.YELLOW+Fore.BLACK+"d"+Back.RESET+Fore.RESET+"=Drain ")
print
...
if NODES_USAGE_BY_DEFAULT ^ options.toggle_nodes and len(jobs)>0:
    ...
    for node in natsorted(nodes_usage.keys()):
        ...
        for job in nodes_usage[node]:
            d=time.strftime("%H:%M:%S",time.gmtime(job["start_time"]+int(job["walltime"])-time.time()))
