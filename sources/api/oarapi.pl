...
my $MAX_ITEMS = 500;
if (is_conf("API_DEFAULT_MAX_ITEMS_NUMBER")) {
    $MAX_ITEMS = get_conf("API_DEFAULT_MAX_ITEMS_NUMBER");
}
...
##############################################################################
  ...
  SWITCH: for ($q) {
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            my $root = {
                "oar_version"    => OAR::Version::get_version(),
                "api_version"    => $VERSION,
                ...
                "links"          => [
                    ...
                    {   'rel'  => 'collection',
                        'href' =>
                          OAR::API::htmlize_uri(OAR::API::make_uri("resources", $ext, 0), $ext),
                        'title' => 'resources'
                    },
                    {   'rel'  => 'collection',
                        'href' => OAR::API::htmlize_uri(
                            OAR::API::make_uri("resources/details", $ext, 0), $ext
                        ),
                    ...
                    {   'rel'  => 'collection',
                        'href' =>
                          OAR::API::htmlize_uri(OAR::API::make_uri("jobs/details", $ext, 0), $ext),
                        'title' => 'detailed_jobs'
                    },
                    {   'rel'  => 'collection',
                        'href' =>
                          OAR::API::htmlize_uri(OAR::API::make_uri("jobs/table", $ext, 0), $ext),
                        'title' => 'jobs_table'
                    },
                    {   'rel'  => 'collection',
                        'href' =>
                          OAR::API::htmlize_uri(OAR::API::make_uri("config", $ext, 0), $ext),
                        'title' => 'config'
                    },
                    {   'rel'  => 'collection',
                        'href' => OAR::API::htmlize_uri(
                            OAR::API::make_uri("admission_rules", $ext, 0), $ext
                        ),
       ...
       #
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            OAR::Stat::open_db_connection or
              OAR::API::ERROR(
                500,
            ...
            my $JOBS_URI_DEFAULT_PARAMS =
              "state=Finishing,Running,Resuming,Suspended,Launching,toLaunch,Waiting,toAckReservation,Hold";
            if (is_conf("API_JOBS_URI_DEFAULT_PARAMS")) {
                $JOBS_URI_DEFAULT_PARAMS = get_conf("API_JOBS_URI_DEFAULT_PARAMS");
            }
            ...
            if (!defined($q->param('from')) &&
                !defined($q->param('to'))    &&
                !defined($q->param('state')) &&
                !defined($q->param('array')) &&
                !defined($q->param('ids'))) {
                my $param = qr{.*from=(.*?)(&|$)};
            ...
            my $jobs =
              OAR::Stat::get_jobs_for_user_query($user, $from, $to, $state, $max_items, $offset,
                $array, \@ids);
            my $total_jobs =
              OAR::Stat::count_jobs_for_user_query($user, $from, $to, $state, $array, \@ids);
                ...
                if (defined($more_infos)) {
                    if ($more_infos eq "/details") {
                        ...
                        foreach my $j (@$jobs) {
                            ...
                            my $resources =
                              OAR::API::struct_job_resources($job_resources, $STRUCTURE);
            ...
            $jobs =
              OAR::API::add_pagination($jobs, $total_jobs, $q->path_info, $q->query_string, $ext,
                $max_items, $offset, $STRUCTURE);
            print $header;
            print $HTML_HEADER if ($ext eq "html");
    ...
    #
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            if (@$job == 0) {
                OAR::Stat::close_db_connection;
                OAR::API::ERROR(404, "Job not found", "Job not found");
                last;
            }
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            $resources =
              OAR::API::add_pagination($resources, @$resources, $q->path_info, undef, $ext, 0, 0,
                $STRUCTURE);
            print $header;
            print $HTML_HEADER if ($ext eq "html");
...
#
        $URI = OAR::API::uri_regex_html_json_yaml(
            'jobs/(array/)?(\d+)/(checkpoints|deletions|holds|rholds|resumptions|resubmissions)/new'
        );
        OAR::API::POST($_, $URI) && do {
            ...
            if ($action eq "resubmissions") {
                if ($? != 0) {
                    my $err = $? >> 8;
                    OAR::API::ERROR(500, "Oar server error", $cmdRes);
                    ...
                    print OAR::API::export(
                        {   'id'    => int($1),
                            'links' => [
                                {   'rel'  => 'self',
                                    'href' => OAR::API::htmlize_uri(
                                        OAR::API::make_uri("jobs/$1", $ext, 0), $ext
                                    )
                                },
                                {   'rel'  => 'parent',
                                    'href' => OAR::API::htmlize_uri(
                                        OAR::API::make_uri("jobs/$jobid", $ext, 0), $ext
                                    ) }
                            ],
                            'status'        => "submitted",
                            'cmd_output'    => "$cmdRes",
        ...
        (OAR::API::PUT($_, $URI) || OAR::API::POST($_, $URI)) && do {
            $_->path_info =~ m/$URI/;
            my $jobid = $1;
            my $ext   = OAR::API::set_ext($q, $2);
            (my $header, my $type) = OAR::API::set_output_format($ext, "GET, POST, PUT, DELETE");
            ...
            # Walltime change
                ...
                OAR::API::ERROR(400, "Bad query",
                    "Could not understand " . $job->{method} . " method");
                last;
            }
            ...
            if ($job->{method} eq "walltime-change") {
                ...
                ($error, $http_status, $status, $message) =
                  OAR::Walltime::request($dbh, $jobid, $authenticated_user, $job->{walltime},
                    $job->{force}, $job->{'delay_next_jobs'},
                    $job->{whole}, $job->{timeout});
        ...
        OAR::API::POST($_, $URI) && do {
            ...
            if ($param_file ne "") {
                my $TMP;
                ($TMP, $tmpparamfilename) =
                  tempfile("oarapi.paramfile.XXXXX", DIR => $TMPDIR, UNLINK => 1);
            ...
            if ($script ne "") {
                my $TMP;
                ($TMP, $tmpfilename) =
                  tempfile("oarapi.subscript.XXXXX", DIR => $TMPDIR, UNLINK => 1);
                print $TMP $script;
                ...
                $cmd =
                  "$OARDODO_CMD bash --noprofile --norc -c \"cp $tmpfilename $directory/ && cd $directory && $oarcmd\"";
            } else {
            ...
            if ($? != 0) {
                ...
                if ($err == 4 ||
                    ...
                    OAR::API::ERROR(401, "Permission denied", "Oardodo error: $err: $cmdRes\n");
                } else {
                    OAR::API::ERROR(500, "Oar server error", $cmdRes);
                }
            } elsif ($cmdRes =~ m/.*JOB_ID\s*=\s*(\d+).*/m) {
                ...
                OAR::API::ERROR(500, "Parse error", "Job submitted but its id could not be parsed");
            }
            last;
        };
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            my $resources;
            OAR::Nodes::open_db_connection or
              OAR::API::ERROR(
                500,
                "Cannot connect to the database",
                "Cannot connect to the database");
            if (defined($1)) {
                if ($1 eq "/full" || $1 eq "/details") {
                    ...
                    if (defined($resource)) {
                        ...
                        OAR::API::ERROR(404, "Resource not found", "Resource not found");
                    }
            ...
            if ($paged == 1) {
                ...
                # add pagination informations
                $resources = OAR::API::add_pagination($resources, $total_resources, $q->path_info,
                    $q->query_string, $ext, $max_items, $offset, $STRUCTURE);
            }
            OAR::Nodes::close_db_connection;
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            OAR::Nodes::close_db_connection;
            $jobs =
              OAR::API::add_pagination($jobs, @$jobs, $q->path_info, undef, $ext, 0, 0, $STRUCTURE);
            print $header;
            print $HTML_HEADER if ($ext eq "html");
            print OAR::API::export($jobs, $ext);
            last;
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            my $dbh = OAR::IO::connect() or
              OAR::API::ERROR(
                ...
                "Cannot connect to the database",
                "Cannot connect to the database");
            my $jobs = OAR::IO::get_resources_jobs($dbh) or
              OAR::IO::disconnect($dbh),
              OAR::API::ERROR(500, "Could not get resources jobs", "Could not get resources jobs");
            my $resources = OAR::IO::get_resources_by_property($dbh, $property) or
              OAR::IO::disconnect($dbh),
              OAR::API::ERROR(
                500,
            ...
            foreach my $property (keys %$resources) {
                if (!defined($resources_by_property_jobs{$property})) {
                    @{ $resources_by_property_jobs{$property} } = ();
                }
            ...
            my $output = {
                ...
                'links' => [
                    {   'rel'  => "self",
                        'href' => OAR::API::htmlize_uri(
                            OAR::API::make_uri("/resources/$property/jobs", $ext, 0), $ext
                        ) }
                ],
                'api_timestamp' => time() };
            OAR::IO::disconnect($dbh);
        ...
        OAR::API::POST($_, $URI) && do {
            ...
            my $dbh = OAR::IO::connect() or
              OAR::API::ERROR(
                ...
                "Cannot connect to the database",
                "Cannot connect to the database");
            my @ids = OAR::IO::add_resources($dbh, $resources) or
              OAR::IO::disconnect($dbh),
              OAR::API::ERROR(
                500,
            ...
            foreach my $id (@ids) {
                ...
                push(@$result,
                    { id => $id, links => [ { rel => "self", href => "resources/$id" } ] });
            }
            $result =
              OAR::API::add_pagination($result, @ids, $q->path_info, $q->query_string, $ext, 0, 0,
                $STRUCTURE);
            print $header;
            print $HTML_HEADER if ($ext eq "html");
            print OAR::API::export($result, $ext);
        ...
        OAR::API::POST($_, $URI) && do {
            ...
            print OAR::API::export(
                {   'status'        => "Change state request registered",
                    ...
                    'api_timestamp' => time(),
                    'uri'           =>
                      OAR::API::htmlize_uri(OAR::API::make_uri("resources/$id", $ext, 0), $ext)
                },
                $ext);
            OAR::Tools::notify_tcp_socket($remote_host, $remote_port, "ChState");
            OAR::Tools::notify_tcp_socket($remote_host, $remote_port, "Term");
        ...
        OAR::API::DELETE($_, $URI) && do {
            ...
            if (not $authenticated_user eq "oar") {
                OAR::API::ERROR(401, "Permission denied", "Only the oar user can delete resources");
                last;
            }
            $ENV{OARDO_BECOME_USER} = "oar";
            ...
            if ($res[0]) { $Resource = $res[0]; }
                ...
                OAR::API::ERROR(404, "Not found",
                    "Corresponding resource could not be found ($id,$node,$cpuset)");
                last;
            }
...
EOS
                ...
                OAR::IO::disconnect($base);
                OAR::API::ERROR(403, "Forbidden",
                    "The resource $Resource must be in the Dead status");
                last;
            }
            OAR::IO::disconnect($base);
            last;
   ...
   #
        ...
        OAR::API::POST($_, $URI) && do {
            ...
            if ($q->param('POSTDATA')) {
                $description =
                  OAR::API::check_resource_description($q->param('POSTDATA'), $q->content_type);

                # getting properties
                if (defined($description->{properties})) {
                    foreach my $property (keys %{ $description->{properties} }) {
                        $cmd_properties .=
                          " -p " . $property . "=" . $description->{properties}->{$property};
                    }
                }
            }
            ...
            $cmd = "$API_RESOURCES_LIST_GENERATION_CMD $auto_offset" .
              $description->{resources} . $cmd_properties;
            ...
            $data =
              OAR::API::add_pagination($data, @$data, $q->path_info, undef, $ext, 0, 0, $STRUCTURE);
            print $header;
            if ($ext eq "html") {
                print $HTML_HEADER;
...
#
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            my $dbh = OAR::IO::connect() or
              OAR::API::ERROR(
                ...
                "Cannot connect to the database",
                "Cannot connect to the database");
            my $accounting = OAR::API::struct_accounting(
                OAR::IO::get_accounting_summary_byproject($dbh, $from, $to, $user, undef, undef));
            my $total;
            ...
            $accounting =
              OAR::API::add_pagination($accounting, $total, $q->path_info, $q->query_string, $ext,
                0, 0, $STRUCTURE);
            print $header;
            print $HTML_HEADER if ($ext eq "html");
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            # add pagination informations
            $admissions_rules =
              OAR::API::add_pagination($admissions_rules, $total_rules, $q->path_info,
                $q->query_string, $ext, $max_items, $offset, $STRUCTURE);
            print $header;
            print $HTML_HEADER if ($ext eq "html");
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            if (defined($admission_rule->{id})) {
                ...
                OAR::API::ERROR(
                    ...
                    "No admission rule corresponding to id=$rule_id");
            }
            last;
        };
        ...
        (OAR::API::POST($_, $URI) || OAR::API::PUT($_, $URI)) && do {
            ...
            if ($q->param('POSTDATA')) {
                ...
                $admission_rule =
                  OAR::API::check_admission_rule($q->param('PUTDATA'), $q->content_type);
            }
            ...
            my $dbh = OAR::IO::connect() or
              OAR::API::ERROR(
                ...
                "Cannot connect to the database",
                "Cannot connect to the database");
            my $id = OAR::IO::add_admission_rule(
                $dbh,
                $admission_rule->{priority},
                $admission_rule->{enabled},
                $admission_rule->{rule});
            if ($id && $id > 0) {
                print $q->header(
                    -status   => 201,
                    -type     => OAR::API::get_content_type($ext),
                    -location => OAR::API::htmlize_uri(
                        OAR::API::make_uri("admission_rules/$id", $ext, 0), $ext
                    ));
        ...
        OAR::API::DELETE($_, $URI) && do {
            ...
            if (defined($admission_rule)) {
                ...
                OAR::Stat::close_db_connection;
                OAR::API::ERROR(404, "Not found",
                    "Corresponding admission rule could not be found");
            }
            last;
        };
        ...
        OAR::API::POST($_, $URI) && do {
            ...
            if (defined($admission_rule->{method}) && $admission_rule->{method} eq "delete") {
            ...
            } elsif (defined($admission_rule->{priority}) and
                defined($admission_rule->{enabled}) and
                defined($admission_rule->{rule})) {
                my $id = OAR::IO::update_admission_rule(
                    $dbh, $rule_id,
                    $admission_rule->{priority},
                    $admission_rule->{enabled},
                    $admission_rule->{rule});
                ...
                OAR::API::ERROR(400, "Bad query",
                    "Could not understand " . $admission_rule->{method} . " method");
            }
            last;
        };
        ...
        OAR::API::POST($_, $URI) && do {
            ...
            if ($q->param('POSTDATA')) {
                $parameter =
                  OAR::API::check_configuration_variable($q->param('POSTDATA'), $q->content_type);
            }
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            print $HTML_HEADER if ($ext eq "html");
            $listing =
              OAR::API::add_pagination($listing, @$listing, $q->path_info, undef, $ext, 0, 0,
                $STRUCTURE);
        ...
        OAR::API::POST($_, $URI) && do {
            ...
            if (not $chmod->{mode} =~ /^[a-z0-9+]+$/) {
                OAR::API::ERROR(401, "Permission denied", "Bad mode value: " . $chmod->{mode});
                last;
            }
            ...
            if ($? != 0) {
                OAR::API::ERROR(500, "Chmod error",
                    "Could not set mode $chmod->{mode} on file $file: $cmdRes");
            }
            print $q->header(
                -status   => 202,
        ...
        OAR::API::POST($_, $URI) && do {
            ...
            if (system("$OARDODO_CMD", "test", "-f", "$file") == 0 and
                ($force eq "" or not defined($force))) {
            ...
            if ($q->content_type =~ /^multipart\/form-data.*/) {
                ...
                OAR::API::ERROR(
                    406,
                    "Bad content type",
                    $q->content_type . " not allowed for file upload");
                last;
            }
            ...
            print OAR::API::export(
                {   'status'  => "created",
                    ...
                    'links'   => [
                        {   "rel"  => "self",
                            "href" => OAR::API::htmlize_uri(
                                OAR::API::make_uri("media/$file", undef, 0), $ext
                            ) } ]
                },
                $ext);
            last;
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            my $dbh = OAR::IO::connect_ro() or
              OAR::API::ERROR(
                500,
                "Cannot connect to the database",
                "Cannot connect to the database");
            my $count = OAR::IO::sql_count($dbh, $query) or
              OAR::IO::disconnect($dbh), OAR::API::ERROR(
                500,
                "SQL error",
                "SQL error"    # <- add here the sql error output
              );
            my $result = OAR::IO::sql_select($dbh, $query, $limit, $offset) or
              OAR::IO::disconnect($dbh), OAR::API::ERROR(
                500,
                "SQL error",
                "SQL error"    # <- add here the sql error output
            ...
            $result =
              OAR::API::add_pagination($result, $count, $q->path_info, $q->query_string, $ext,
...
#
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            if (is_conf("API_COLMET_EXTRACT_PATH")) {
                $COLMET_EXTRACT_PATH = get_conf("API_COLMET_EXTRACT_PATH");
            }
            my $COLMET_HDF5_PATH_PREFIX = "/var/lib/colmet/hdf5/data";
            if (is_conf("API_COLMET_HDF5_PATH_PREFIX")) {
                $COLMET_HDF5_PATH_PREFIX = get_conf("API_COLMET_HDF5_PATH_PREFIX");
            }

            if (not -X $COLMET_EXTRACT_PATH) {
                OAR::API::ERROR(
                    400,
                    "Missing extractor script",
            ...
            my $cmd =
              "$COLMET_EXTRACT_PATH $COLMET_HDF5_PATH_PREFIX $jobid $start_time $stop_time $metrics";
            my $cmdRes = OAR::API::send_cmd($cmd, "Oar");
        ...
        OAR::API::GET($_, $URI) && do {
            ...
            if (is_conf("API_STRESS_FACTOR_SCRIPT")) {
                $stress_factor_script = get_conf("API_STRESS_FACTOR_SCRIPT");
            }
            ...
            my %output = (
                ...
                "links"         => [
                    {   'rel'  => 'self',
                        'href' =>
                          OAR::API::htmlize_uri(OAR::API::make_uri("stress_factor", $ext, 0), $ext)
                    } ]);
